...
/**
 * PHPExcel1_Writer_Excel2007_ContentTypes
 *
 ...
 */
class PHPExcel1_Writer_Excel2007_ContentTypes extends PHPExcel1_Writer_Excel2007_WriterPart
{
    /**
     ...
     */
    public function writeContentTypes(PHPExcel1 $pPHPExcel1 = null, $includeCharts = false)
    {
        // Create XML writer
        $objWriter = null;
        ...
        // Workbook
        if ($pPHPExcel1->hasMacros()) { //Macros in workbook ?
            ...
            if ($pPHPExcel1->hasMacrosCertificate()) {// signed macros ?
        ...
        $chart = 1;
        for ($i = 0; $i < $sheetCount; ++$i) {
            $drawings = $pPHPExcel1->getSheet($i)->getDrawingCollection();
            $drawingCount = count($drawings);
            $chartCount = ($includeCharts) ? $pPHPExcel1->getSheet($i)->getChartCount() : 0;
        ...
        // Comments
        for ($i = 0; $i < $sheetCount; ++$i) {
            if (count($pPHPExcel1->getSheet($i)->getComments()) > 0) {
        ...
        for ($i = 0; $i < $mediaCount; ++$i) {
            ...
            if ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof PHPExcel1_Worksheet_Drawing) {
                $extension = strtolower($this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getExtension());
                $mimeType = $this->getImageMimeType($this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getPath());
            } elseif ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof PHPExcel1_Worksheet_MemoryDrawing) {
        ...
        if ($pPHPExcel1->hasRibbonBinObjects()) {
            // Some additional objects in the ribbon ?
            // we need to write "Extension" but not already write for media content
            $tabRibbonTypes=array_diff($pPHPExcel1->getRibbonBinObjects('types'), array_keys($aMediaContentTypes));
        ...
        $sheetCount = $pPHPExcel1->getSheetCount();
        for ($i = 0; $i < $sheetCount; ++$i) {
            if (count($pPHPExcel1->getSheet()->getHeaderFooter()->getImages()) > 0) {
                foreach ($pPHPExcel1->getSheet()->getHeaderFooter()->getImages() as $image) {
    ...
    {
        if (PHPExcel1_Shared_File::file_exists($pFile)) {
            $image = getimagesize($pFile);
            return image_type_to_mime_type($image[2]);
        } else {
            throw new PHPExcel1_Writer_Exception("File $pFile does not exist");
        }
    }
     ...
     * @throws     PHPExcel1_Writer_Exception
     */
    private function writeDefaultContentType(PHPExcel1_Shared_XMLWriter $objWriter = null, $pPartname = '', $pContentType = '')
    {
     ...
     * @throws     PHPExcel1_Writer_Exception
     */
    private function writeOverrideContentType(PHPExcel1_Shared_XMLWriter $objWriter = null, $pPartname = '', $pContentType = '')
    {
        if ($pPartname != '' && $pContentType != '') {
            // Write content type
