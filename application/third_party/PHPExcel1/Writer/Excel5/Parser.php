...
/**
 * PHPExcel1_Writer_Excel5_Parser
 *
...
// */
class PHPExcel1_Writer_Excel5_Parser
{
    /**    Constants                */
    ...
    private function convert($token)
    {
        ...
        throw new PHPExcel1_Writer_Exception("Unknown token $token");
    }
    ...
    private function convertNumber($num)
    {
        ...
        if ((preg_match("/^\d+$/", $num)) and ($num <= 65535)) {
            ...
            if (PHPExcel1_Writer_Excel5_BIFFwriter::getByteOrder()) { // if it's Big Endian
                $num = strrev($num);
            }
    ...
    private function convertString($string)
    {
        ...
        if (strlen($string) > 255) {
            throw new PHPExcel1_Writer_Exception("String is too long");
        }

        return pack('C', $this->ptg['ptgStr']) . PHPExcel1_Shared_String::UTF8toBIFF8UnicodeShort($string);
    }
...
//        $volatile = $this->functions[$token][3];
    ...
    private function convertRange2d($range, $class = 0)
    {
        ...
        if (preg_match('/^(\$)?([A-Ia-i]?[A-Za-z])(\$)?(\d+)\:(\$)?([A-Ia-i]?[A-Za-z])(\$)?(\d+)$/', $range)) {
            ...
            // TODO: use real error codes
            throw new PHPExcel1_Writer_Exception("Unknown range separator");
        }
        ...
        if ($class == 0) {
            ...
            // TODO: use real error codes
            throw new PHPExcel1_Writer_Exception("Unknown class $class");
        }
...
//        }
    ...
    private function packExtRef($ext_ref)
    {
        ...
        if (preg_match("/:/", $ext_ref)) {
            ...
            if ($sheet1 == -1) {
                throw new PHPExcel1_Writer_Exception("Unknown sheet name $sheet_name1 in formula");
            }
            $sheet2 = $this->getSheetIndex($sheet_name2);
            if ($sheet2 == -1) {
                throw new PHPExcel1_Writer_Exception("Unknown sheet name $sheet_name2 in formula");
            }
            ...
            if ($sheet1 == -1) {
                throw new PHPExcel1_Writer_Exception("Unknown sheet name $ext_ref in formula");
            }
            $sheet2 = $sheet1;
        }
     ...
     * PHPExcel1_Writer_Excel5_Workbook class.
     *
     * @access    private
     ...
     *
     * @access public
     * @see PHPExcel1_Writer_Excel5_Workbook::addWorksheet()
    ...
    private function cellToPackedRowcol($cell)
    {
        ...
        if ($col >= 256) {
            throw new PHPExcel1_Writer_Exception("Column in: $cell greater than 255");
        }
        if ($row >= 65536) {
            throw new PHPExcel1_Writer_Exception("Row in: $cell greater than 65536 ");
        }
    ...
    private function rangeToPackedRange($range)
    {
        ...
        if (($row1 >= 65536) or ($row2 >= 65536)) {
            throw new PHPExcel1_Writer_Exception("Row in: $range greater than 65536 ");
        }
    ...
    private function fact()
    {
        if ($this->currentToken == "(") {
            ...
            if ($this->currentToken != ")") {
                throw new PHPExcel1_Writer_Exception("')' token expected.");
            }
            $this->advance();         // eat the ")"
            return $result;
        }
        ...
        throw new PHPExcel1_Writer_Exception("Syntax error: ".$this->currentToken.", lookahead: ".$this->lookAhead.", current char: ".$this->currentCharacter);
    }
    ...
    private function func()
    {
        ...
        /**/
            if ($num_args > 0) {
                if ($this->currentToken == "," || $this->currentToken == ";") {
                    ...
                    throw new PHPExcel1_Writer_Exception("Syntax error: comma expected in function $function, arg #{$num_args}");
                }
        ...
        if (!isset($this->functions[$function])) {
            throw new PHPExcel1_Writer_Exception("Function $function() doesn't exist");
        }
        $args = $this->functions[$function][1];
        // If fixed number of args eg. TIME($i, $j, $k). Check that the number of args is valid.
        if (($args >= 0) and ($args != $num_args)) {
            throw new PHPExcel1_Writer_Exception("Incorrect number of arguments in function $function() ");
        }
