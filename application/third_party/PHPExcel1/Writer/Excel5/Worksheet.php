...
/**
 * PHPExcel1_Writer_Excel5_Worksheet
 *
 ...
 *
 * @category   PHPExcel1
 * @package    PHPExcel1_Writer_Excel5
...
// */
class PHPExcel1_Writer_Excel5_Worksheet extends PHPExcel1_Writer_Excel5_BIFFwriter
{
    /**
     * Formula parser
     *
     * @var PHPExcel1_Writer_Excel5_Parser
     */
    private $parser;
     ...
     * @param PHPExcel1_Worksheet $phpSheet
     */
...
//        $this->firstRowIndex = ($minR > 65535) ? 65535 : $minR;
        ...
        $this->firstColumnIndex = PHPExcel1_Cell::columnIndexFromString($minC);
        $this->lastColumnIndex  = PHPExcel1_Cell::columnIndexFromString($maxC);
...
//        if ($this->firstColumnIndex > 255) $this->firstColumnIndex = 255;
     ...
     *
     * @access public
     * @see PHPExcel1_Writer_Excel5_Workbook::storeWorkbook()
     */
    public function close()
    {
        ...
        if (($defaultWidth = $phpSheet->getDefaultColumnDimension()->getWidth()) < 0) {
            $defaultWidth = PHPExcel1_Shared_Font::getDefaultColumnWidthByFont($phpSheet->getParent()->getDefaultStyle()->getFont());
        }
        ...
        for ($i = 0; $i <= $maxCol; ++$i) {
            ...
            $columnLetter = PHPExcel1_Cell::stringFromColumnIndex($i);
        ...
        foreach ($phpSheet->getCellCollection() as $cellID) {
            ...
            $row = $cell->getRow() - 1;
            $column = PHPExcel1_Cell::columnIndexFromString($cell->getColumn()) - 1;
...
//            if ($row + 1 > 65536 or $column + 1 > 256) {
            ...
            $cVal = $cell->getValue();
            if ($cVal instanceof PHPExcel1_RichText) {
                // $this->writeString($row, $column, $cVal->getPlainText(), $xfIndex);
                $arrcRun = array();
                $str_len = PHPExcel1_Shared_String::CountCharacters($cVal->getPlainText(), 'UTF-8');
                $str_pos = 0;
                $elements = $cVal->getRichTextElements();
                foreach ($elements as $element) {
                    // FONT Index
                    if ($element instanceof PHPExcel1_RichText_Run) {
                    ...
                    // Position FROM
                    $str_pos += PHPExcel1_Shared_String::CountCharacters($element->getText(), 'UTF-8');
                }
                $this->writeRichTextString($row, $column, $cVal->getPlainText(), $xfIndex, $arrcRun);
            } else {
                switch ($cell->getDatatype()) {
                    case PHPExcel1_Cell_DataType::TYPE_STRING:
                    case PHPExcel1_Cell_DataType::TYPE_NULL:
                        if ($cVal === '' || $cVal === null) {
        ...
        foreach ($phpSheet->getHyperLinkCollection() as $coordinate => $hyperlink) {
            list($column, $row) = PHPExcel1_Cell::coordinateFromString($coordinate);
            ...
            $this->writeUrl($row - 1, PHPExcel1_Cell::columnIndexFromString($column) - 1, $url);
        }
        ...
        if (!empty($arrConditionalStyles)) {
            ...
            foreach ($arrConditionalStyles as $cellCoordinate => $conditionalStyles) {
                foreach ($conditionalStyles as $conditional) {
                    if ($conditional->getConditionType() == PHPExcel1_Style_Conditional::CONDITION_EXPRESSION
                        || $conditional->getConditionType() == PHPExcel1_Style_Conditional::CONDITION_CELLIS) {
    ...
    private function writeBIFF8CellRangeAddressFixed($range = 'A1')
    {
        ...
        $firstCellCoordinates = PHPExcel1_Cell::coordinateFromString($firstCell); // e.g. array(0, 1)
        $lastCellCoordinates  = PHPExcel1_Cell::coordinateFromString($lastCell);  // e.g. array(1, 6)

        return pack('vvvv', $firstCellCoordinates[1] - 1, $lastCellCoordinates[1] - 1, PHPExcel1_Cell::columnIndexFromString($firstCellCoordinates[0]) - 1, PHPExcel1_Cell::columnIndexFromString($lastCellCoordinates[0]) - 1);
    }
    ...
    private function writeRichTextString($row, $col, $str, $xfIndex, $arrcRun)
    {
        ...
        $str = PHPExcel1_Shared_String::UTF8toBIFF8UnicodeShort($str, $arrcRun);
    ...
    private function writeLabelSst($row, $col, $str, $xfIndex)
    {
        ...
        $str = PHPExcel1_Shared_String::UTF8toBIFF8UnicodeLong($str);
    ...
    private function writeFormula($row, $col, $formula, $xfIndex, $calculatedValue)
    {
        ...
        if (isset($calculatedValue)) {
            ...
            if (is_bool($calculatedValue)) {
                ...
                if (array_key_exists($calculatedValue, PHPExcel1_Cell_DataType::getErrorCodes())) {
                    // Error value
    ...
    private function writeStringRecord($stringValue)
    {
        ...
        $data = PHPExcel1_Shared_String::UTF8toBIFF8UnicodeLong($stringValue);
    ...
    public function writeUrlInternal($row1, $col1, $row2, $col2, $url)
    {
        ...
        // character count
        $url_len = PHPExcel1_Shared_String::CountCharacters($url);
        $url_len = pack('V', $url_len);

        $url = PHPExcel1_Shared_String::ConvertEncoding($url, 'UTF-16LE', 'UTF-8');
    ...
    private function writeWindow2()
    {
        ...
        // no support in PHPExcel1 for selected sheet, therefore sheet is only selected if it is the active sheet
        $fSelected      = ($this->phpSheet === $this->phpSheet->getParent()->getActiveSheet()) ? 1 : 0;
        $fPaged         = 1;                     // 2
        $fPageBreakPreview = $this->phpSheet->getSheetView()->getView() === PHPExcel1_Worksheet_SheetView::SHEETVIEW_PAGE_BREAK_PREVIEW;
    ...
    private function writeSelection()
    {
        ...
        $selectedCells = PHPExcel1_Cell::splitRange($this->phpSheet->getSelectedCells());
        ...
        list($colFirst, $rwFirst) = PHPExcel1_Cell::coordinateFromString($first);
        $colFirst = PHPExcel1_Cell::columnIndexFromString($colFirst) - 1; // base 0 column index
        --$rwFirst; // base 0 row index

        list($colLast, $rwLast) = PHPExcel1_Cell::coordinateFromString($last);
        $colLast = PHPExcel1_Cell::columnIndexFromString($colLast) - 1; // base 0 column index
    ...
    private function writeMergedCells()
    {
        ...
        foreach ($mergeCells as $mergeCell) {
            ...
            $range = PHPExcel1_Cell::splitRange($mergeCell);
            list($first, $last) = $range[0];
            list($firstColumn, $firstRow) = PHPExcel1_Cell::coordinateFromString($first);
            list($lastColumn, $lastRow) = PHPExcel1_Cell::coordinateFromString($last);

            $recordData .= pack('vvvv', $firstRow - 1, $lastRow - 1, PHPExcel1_Cell::columnIndexFromString($firstColumn) - 1, PHPExcel1_Cell::columnIndexFromString($lastColumn) - 1);
    ...
    private function writeRangeProtection()
    {
        foreach ($this->phpSheet->getProtectedCells() as $range => $password) {
            ...
            $recordData .= PHPExcel1_Shared_String::UTF8toBIFF8UnicodeLong('p' . md5($recordData));
    ...
    private function writePanes()
    {
        ...
        if ($freezePane = $this->phpSheet->getFreezePane()) {
            list($column, $row) = PHPExcel1_Cell::coordinateFromString($freezePane);
            $panes[0] = $row - 1;
            $panes[1] = PHPExcel1_Cell::columnIndexFromString($column) - 1;
        } else {
            // thaw panes
            return;
        }
    ...
    private function writeSetup()
    {
        ...
        // Page orientation
        $fLandscape = ($this->phpSheet->getPageSetup()->getOrientation() == PHPExcel1_Worksheet_PageSetup::ORIENTATION_LANDSCAPE) ?
            0x0 : 0x1;
    ...
    private function writeHeader()
    {
        ...
        */
        ...
        $recordData = PHPExcel1_Shared_String::UTF8toBIFF8UnicodeLong($this->phpSheet->getHeaderFooter()->getOddHeader());
        $length = strlen($recordData);
    ...
    private function writeFooter()
    {
        ...
        */
        ...
        $recordData = PHPExcel1_Shared_String::UTF8toBIFF8UnicodeLong($this->phpSheet->getHeaderFooter()->getOddFooter());
        $length = strlen($recordData);
    ...
    private function writeAutoFilterInfo()
    {
        ...
        $rangeBounds = PHPExcel1_Cell::rangeBoundaries($this->phpSheet->getAutoFilter()->getRange());
    ...
    private function writeBreaks()
    {
        ...
        foreach ($this->phpSheet->getBreaks() as $cell => $breakType) {
            // Fetch coordinates
            $coordinates = PHPExcel1_Cell::coordinateFromString($cell);

            // Decide what to do by the type of break
            switch ($breakType) {
                case PHPExcel1_Worksheet::BREAK_COLUMN:
                    // Add to list of vertical breaks
                    $vbreaks[] = PHPExcel1_Cell::columnIndexFromString($coordinates[0]) - 1;
                    break;
                case PHPExcel1_Worksheet::BREAK_ROW:
                    // Add to list of horizontal breaks
                    $hbreaks[] = $coordinates[1];
                    break;
                case PHPExcel1_Worksheet::BREAK_NONE:
                default:
                    // Nothing to do
                    break;
            }
    ...
    public function positionImage($col_start, $row_start, $x1, $y1, $width, $height)
    {
        ...
        if ($x1 >= PHPExcel1_Shared_Excel5::sizeCol($this->phpSheet, PHPExcel1_Cell::stringFromColumnIndex($col_start))) {
            $x1 = 0;
        }
        if ($y1 >= PHPExcel1_Shared_Excel5::sizeRow($this->phpSheet, $row_start + 1)) {
            $y1 = 0;
        }
        ...
        while ($width >= PHPExcel1_Shared_Excel5::sizeCol($this->phpSheet, PHPExcel1_Cell::stringFromColumnIndex($col_end))) {
            $width -= PHPExcel1_Shared_Excel5::sizeCol($this->phpSheet, PHPExcel1_Cell::stringFromColumnIndex($col_end));
            ++$col_end;
        }

        // Subtract the underlying cell heights to find the end cell of the image
        while ($height >= PHPExcel1_Shared_Excel5::sizeRow($this->phpSheet, $row_end + 1)) {
            $height -= PHPExcel1_Shared_Excel5::sizeRow($this->phpSheet, $row_end + 1);
            ++$row_end;
        }
        ...
        //
        if (PHPExcel1_Shared_Excel5::sizeCol($this->phpSheet, PHPExcel1_Cell::stringFromColumnIndex($col_start)) == 0) {
            return;
        }
        if (PHPExcel1_Shared_Excel5::sizeCol($this->phpSheet, PHPExcel1_Cell::stringFromColumnIndex($col_end))   == 0) {
            return;
        }
        if (PHPExcel1_Shared_Excel5::sizeRow($this->phpSheet, $row_start + 1) == 0) {
            return;
        }
        if (PHPExcel1_Shared_Excel5::sizeRow($this->phpSheet, $row_end + 1)   == 0) {
            return;
        }

        // Convert the pixel values to the percentage value expected by Excel
        $x1 = $x1     / PHPExcel1_Shared_Excel5::sizeCol($this->phpSheet, PHPExcel1_Cell::stringFromColumnIndex($col_start))   * 1024;
        $y1 = $y1     / PHPExcel1_Shared_Excel5::sizeRow($this->phpSheet, $row_start + 1)   *  256;
        $x2 = $width  / PHPExcel1_Shared_Excel5::sizeCol($this->phpSheet, PHPExcel1_Cell::stringFromColumnIndex($col_end))     * 1024; // Distance to right side of object
        $y2 = $height / PHPExcel1_Shared_Excel5::sizeRow($this->phpSheet, $row_end + 1)     *  256; // Distance to bottom of object
    ...
    public function processBitmap($bitmap)
    {
        ...
        if (!$bmp_fd) {
            throw new PHPExcel1_Writer_Exception("Couldn't import $bitmap");
        }
        ...
        if (strlen($data) <= 0x36) {
            throw new PHPExcel1_Writer_Exception("$bitmap doesn't contain enough data.\n");
        }

        // The first 2 bytes are used to identify the bitmap.
        $identity = unpack("A2ident", $data);
        if ($identity['ident'] != "BM") {
            throw new PHPExcel1_Writer_Exception("$bitmap doesn't appear to be a valid bitmap image.\n");
        }
        ...
        $data   = substr($data, 8);
        if ($width > 0xFFFF) {
            throw new PHPExcel1_Writer_Exception("$bitmap: largest image width supported is 65k.\n");
        }
        if ($height > 0xFFFF) {
            throw new PHPExcel1_Writer_Exception("$bitmap: largest image height supported is 65k.\n");
        }
        ...
        if ($planes_and_bitcount[2] != 24) { // Bitcount
            throw new PHPExcel1_Writer_Exception("$bitmap isn't a 24bit true color bitmap.\n");
        }
        if ($planes_and_bitcount[1] != 1) {
            throw new PHPExcel1_Writer_Exception("$bitmap: only 1 plane supported in bitmap image.\n");
        }
        ...
        if ($compression['comp'] != 0) {
            throw new PHPExcel1_Writer_Exception("$bitmap: compression not supported in bitmap image.\n");
        }
     ...
     * Get Escher object
     *
     * @return PHPExcel1_Shared_Escher
     */
     ...
     * Set Escher object
     *
     * @param PHPExcel1_Shared_Escher $pValue
     */
    public function setEscher(PHPExcel1_Shared_Escher $pValue = null)
    {
        $this->escher = $pValue;
    }
    ...
    private function writeMsoDrawing()
    {
        ...
        if (isset($this->escher)) {
            $writer = new PHPExcel1_Writer_Excel5_Escher($this->escher);
            $data = $writer->close();
    ...
    private function writeDataValidity()
    {
        ...
        if (!empty($dataValidationCollection)) {
            ...
            foreach ($dataValidationCollection as $cellCoordinate => $dataValidation) {
                ...
                switch ($type) {
                    case PHPExcel1_Cell_DataValidation::TYPE_NONE:
                        $type = 0x00;
                        break;
                    case PHPExcel1_Cell_DataValidation::TYPE_WHOLE:
                        $type = 0x01;
                        break;
                    case PHPExcel1_Cell_DataValidation::TYPE_DECIMAL:
                        $type = 0x02;
                        break;
                    case PHPExcel1_Cell_DataValidation::TYPE_LIST:
                        $type = 0x03;
                        break;
                    case PHPExcel1_Cell_DataValidation::TYPE_DATE:
                        $type = 0x04;
                        break;
                    case PHPExcel1_Cell_DataValidation::TYPE_TIME:
                        $type = 0x05;
                        break;
                    case PHPExcel1_Cell_DataValidation::TYPE_TEXTLENGTH:
                        $type = 0x06;
                        break;
                    case PHPExcel1_Cell_DataValidation::TYPE_CUSTOM:
                        $type = 0x07;
                        break;
                }
                $options |= $type << 0;
                ...
                switch ($errorStyle) {
                    case PHPExcel1_Cell_DataValidation::STYLE_STOP:
                        $errorStyle = 0x00;
                        break;
                    case PHPExcel1_Cell_DataValidation::STYLE_WARNING:
                        $errorStyle = 0x01;
                        break;
                    case PHPExcel1_Cell_DataValidation::STYLE_INFORMATION:
                        $errorStyle = 0x02;
                        break;
                }
                ...
                switch ($operator) {
                    case PHPExcel1_Cell_DataValidation::OPERATOR_BETWEEN:
                        $operator = 0x00;
                        break;
                    case PHPExcel1_Cell_DataValidation::OPERATOR_NOTBETWEEN:
                        $operator = 0x01;
                        break;
                    case PHPExcel1_Cell_DataValidation::OPERATOR_EQUAL:
                        $operator = 0x02;
                        break;
                    case PHPExcel1_Cell_DataValidation::OPERATOR_NOTEQUAL:
                        $operator = 0x03;
                        break;
                    case PHPExcel1_Cell_DataValidation::OPERATOR_GREATERTHAN:
                        $operator = 0x04;
                        break;
                    case PHPExcel1_Cell_DataValidation::OPERATOR_LESSTHAN:
                        $operator = 0x05;
                        break;
                    case PHPExcel1_Cell_DataValidation::OPERATOR_GREATERTHANOREQUAL:
                        $operator = 0x06;
                        break;
                    case PHPExcel1_Cell_DataValidation::OPERATOR_LESSTHANOREQUAL:
                        $operator = 0x07;
                        break;
                }
                $options |= $operator << 20;
                ...
                $data .= PHPExcel1_Shared_String::UTF8toBIFF8UnicodeLong($promptTitle);

                // error title
                $errorTitle = $dataValidation->getErrorTitle() !== '' ?
                    $dataValidation->getErrorTitle() : chr(0);
                $data .= PHPExcel1_Shared_String::UTF8toBIFF8UnicodeLong($errorTitle);

                // prompt text
                $prompt = $dataValidation->getPrompt() !== '' ?
                    $dataValidation->getPrompt() : chr(0);
                $data .= PHPExcel1_Shared_String::UTF8toBIFF8UnicodeLong($prompt);

                // error text
                $error = $dataValidation->getError() !== '' ?
                    $dataValidation->getError() : chr(0);
                $data .= PHPExcel1_Shared_String::UTF8toBIFF8UnicodeLong($error);
                ...
                try {
                    ...
                    if ($formula2 === '') {
                        throw new PHPExcel1_Writer_Exception('No formula2');
                    }
    ...
    /**
     * Write CFRule Record
     * @param PHPExcel1_Style_Conditional $conditional
     */
    private function writeCFRule(PHPExcel1_Style_Conditional $conditional)
    {
        ...
        if ($conditional->getConditionType() == PHPExcel1_Style_Conditional::CONDITION_EXPRESSION) {
            $type = 0x02;
            $operatorType = 0x00;
        } elseif ($conditional->getConditionType() == PHPExcel1_Style_Conditional::CONDITION_CELLIS) {
            $type = 0x01;

            switch ($conditional->getOperatorType()) {
                case PHPExcel1_Style_Conditional::OPERATOR_NONE:
                    $operatorType = 0x00;
                    break;
                case PHPExcel1_Style_Conditional::OPERATOR_EQUAL:
                    $operatorType = 0x03;
                    break;
                case PHPExcel1_Style_Conditional::OPERATOR_GREATERTHAN:
                    $operatorType = 0x05;
                    break;
                case PHPExcel1_Style_Conditional::OPERATOR_GREATERTHANOREQUAL:
                    $operatorType = 0x07;
                    break;
                case PHPExcel1_Style_Conditional::OPERATOR_LESSTHAN:
                    $operatorType = 0x06;
                    break;
                case PHPExcel1_Style_Conditional::OPERATOR_LESSTHANOREQUAL:
                    $operatorType = 0x08;
                    break;
                case PHPExcel1_Style_Conditional::OPERATOR_NOTEQUAL:
                    $operatorType = 0x04;
                    break;
                case PHPExcel1_Style_Conditional::OPERATOR_BETWEEN:
                    $operatorType = 0x01;
                    break;
        ...
        if ($numConditions == 1) {
            ...
            $operand2 = null;
        } elseif ($numConditions == 2 && ($conditional->getOperatorType() == PHPExcel1_Style_Conditional::OPERATOR_BETWEEN)) {
            $szValue1 = ($arrConditions[0] <= 65535 ? 3 : 0x0000);
        ...
        // Border
        $bBorderLeft = ($conditional->getStyle()->getBorders()->getLeft()->getColor()->getARGB() == PHPExcel1_Style_Color::COLOR_BLACK
                        && $conditional->getStyle()->getBorders()->getLeft()->getBorderStyle() == PHPExcel1_Style_Border::BORDER_NONE ? 1 : 0);
        $bBorderRight = ($conditional->getStyle()->getBorders()->getRight()->getColor()->getARGB() == PHPExcel1_Style_Color::COLOR_BLACK
                        && $conditional->getStyle()->getBorders()->getRight()->getBorderStyle() == PHPExcel1_Style_Border::BORDER_NONE ? 1 : 0);
        $bBorderTop = ($conditional->getStyle()->getBorders()->getTop()->getColor()->getARGB() == PHPExcel1_Style_Color::COLOR_BLACK
                        && $conditional->getStyle()->getBorders()->getTop()->getBorderStyle() == PHPExcel1_Style_Border::BORDER_NONE ? 1 : 0);
        $bBorderBottom = ($conditional->getStyle()->getBorders()->getBottom()->getColor()->getARGB() == PHPExcel1_Style_Color::COLOR_BLACK
                        && $conditional->getStyle()->getBorders()->getBottom()->getBorderStyle() == PHPExcel1_Style_Border::BORDER_NONE ? 1 : 0);
        ...
        if ($bFormatFont == 1) {
            ...
            if ($conditional->getStyle()->getFont()->getName() == null) {
                ...
                $dataBlockFont = PHPExcel1_Shared_String::UTF8toBIFF8UnicodeLong($conditional->getStyle()->getFont()->getName());
            }
            // Font Size
            ...
            switch ($conditional->getStyle()->getFont()->getUnderline()) {
                case PHPExcel1_Style_Font::UNDERLINE_NONE:
                    $dataBlockFont .= pack('C', 0x00);
                    $fontUnderline = 0;
                    break;
                case PHPExcel1_Style_Font::UNDERLINE_DOUBLE:
                    $dataBlockFont .= pack('C', 0x02);
                    $fontUnderline = 0;
                    break;
                case PHPExcel1_Style_Font::UNDERLINE_DOUBLEACCOUNTING:
                    $dataBlockFont .= pack('C', 0x22);
                    $fontUnderline = 0;
                    break;
                case PHPExcel1_Style_Font::UNDERLINE_SINGLE:
                    $dataBlockFont .= pack('C', 0x01);
                    $fontUnderline = 0;
                    break;
                case PHPExcel1_Style_Font::UNDERLINE_SINGLEACCOUNTING:
        ...
        if ($bFormatAlign == 1) {
            ...
            switch ($conditional->getStyle()->getAlignment()->getHorizontal()) {
                case PHPExcel1_Style_Alignment::HORIZONTAL_GENERAL:
                    $blockAlign = 0;
                    break;
                case PHPExcel1_Style_Alignment::HORIZONTAL_LEFT:
                    $blockAlign = 1;
                    break;
                case PHPExcel1_Style_Alignment::HORIZONTAL_RIGHT:
                    $blockAlign = 3;
                    break;
                case PHPExcel1_Style_Alignment::HORIZONTAL_CENTER:
                    $blockAlign = 2;
                    break;
                case PHPExcel1_Style_Alignment::HORIZONTAL_CENTER_CONTINUOUS:
                    $blockAlign = 6;
                    break;
                case PHPExcel1_Style_Alignment::HORIZONTAL_JUSTIFY:
                    $blockAlign = 5;
                    break;
            }
            ...
            switch ($conditional->getStyle()->getAlignment()->getVertical()) {
                case PHPExcel1_Style_Alignment::VERTICAL_BOTTOM:
                    $blockAlign = 2 << 4;
                    break;
                case PHPExcel1_Style_Alignment::VERTICAL_TOP:
                    $blockAlign = 0 << 4;
                    break;
                case PHPExcel1_Style_Alignment::VERTICAL_CENTER:
                    $blockAlign = 1 << 4;
                    break;
                case PHPExcel1_Style_Alignment::VERTICAL_JUSTIFY:
                    $blockAlign = 3 << 4;
                    break;
            }
            $blockAlign |= 0 << 7;
        ...
        if ($bFormatBorder == 1) {
            ...
            switch ($conditional->getStyle()->getBorders()->getLeft()->getBorderStyle()) {
                case PHPExcel1_Style_Border::BORDER_NONE:
                    $blockLineStyle |= 0x00;
                    break;
                case PHPExcel1_Style_Border::BORDER_THIN:
                    $blockLineStyle |= 0x01;
                    break;
                case PHPExcel1_Style_Border::BORDER_MEDIUM:
                    $blockLineStyle |= 0x02;
                    break;
                case PHPExcel1_Style_Border::BORDER_DASHED:
                    $blockLineStyle |= 0x03;
                    break;
                case PHPExcel1_Style_Border::BORDER_DOTTED:
                    $blockLineStyle |= 0x04;
                    break;
                case PHPExcel1_Style_Border::BORDER_THICK:
                    $blockLineStyle |= 0x05;
                    break;
                case PHPExcel1_Style_Border::BORDER_DOUBLE:
                    $blockLineStyle |= 0x06;
                    break;
                case PHPExcel1_Style_Border::BORDER_HAIR:
                    $blockLineStyle |= 0x07;
                    break;
                case PHPExcel1_Style_Border::BORDER_MEDIUMDASHED:
                    $blockLineStyle |= 0x08;
                    break;
                case PHPExcel1_Style_Border::BORDER_DASHDOT:
                    $blockLineStyle |= 0x09;
                    break;
                case PHPExcel1_Style_Border::BORDER_MEDIUMDASHDOT:
                    $blockLineStyle |= 0x0A;
                    break;
                case PHPExcel1_Style_Border::BORDER_DASHDOTDOT:
                    $blockLineStyle |= 0x0B;
                    break;
                case PHPExcel1_Style_Border::BORDER_MEDIUMDASHDOTDOT:
                    $blockLineStyle |= 0x0C;
                    break;
                case PHPExcel1_Style_Border::BORDER_SLANTDASHDOT:
                    $blockLineStyle |= 0x0D;
                    break;
            }
        ...
        if ($bFormatFill == 1) {
            ...
            switch ($conditional->getStyle()->getFill()->getFillType()) {
                case PHPExcel1_Style_Fill::FILL_NONE:
                    $blockFillPatternStyle = 0x00;
                    break;
                case PHPExcel1_Style_Fill::FILL_SOLID:
                    $blockFillPatternStyle = 0x01;
                    break;
                case PHPExcel1_Style_Fill::FILL_PATTERN_MEDIUMGRAY:
                    $blockFillPatternStyle = 0x02;
                    break;
                case PHPExcel1_Style_Fill::FILL_PATTERN_DARKGRAY:
                    $blockFillPatternStyle = 0x03;
                    break;
                case PHPExcel1_Style_Fill::FILL_PATTERN_LIGHTGRAY:
                    $blockFillPatternStyle = 0x04;
                    break;
                case PHPExcel1_Style_Fill::FILL_PATTERN_DARKHORIZONTAL:
                    $blockFillPatternStyle = 0x05;
                    break;
                case PHPExcel1_Style_Fill::FILL_PATTERN_DARKVERTICAL:
                    $blockFillPatternStyle = 0x06;
                    break;
                case PHPExcel1_Style_Fill::FILL_PATTERN_DARKDOWN:
                    $blockFillPatternStyle = 0x07;
                    break;
                case PHPExcel1_Style_Fill::FILL_PATTERN_DARKUP:
                    $blockFillPatternStyle = 0x08;
                    break;
                case PHPExcel1_Style_Fill::FILL_PATTERN_DARKGRID:
                    $blockFillPatternStyle = 0x09;
                    break;
                case PHPExcel1_Style_Fill::FILL_PATTERN_DARKTRELLIS:
                    $blockFillPatternStyle = 0x0A;
                    break;
                case PHPExcel1_Style_Fill::FILL_PATTERN_LIGHTHORIZONTAL:
                    $blockFillPatternStyle = 0x0B;
                    break;
                case PHPExcel1_Style_Fill::FILL_PATTERN_LIGHTVERTICAL:
                    $blockFillPatternStyle = 0x0C;
                    break;
                case PHPExcel1_Style_Fill::FILL_PATTERN_LIGHTDOWN:
                    $blockFillPatternStyle = 0x0D;
                    break;
                case PHPExcel1_Style_Fill::FILL_PATTERN_LIGHTUP:
                    $blockFillPatternStyle = 0x0E;
                    break;
                case PHPExcel1_Style_Fill::FILL_PATTERN_LIGHTGRID:
                    $blockFillPatternStyle = 0x0F;
                    break;
                case PHPExcel1_Style_Fill::FILL_PATTERN_LIGHTTRELLIS:
                    $blockFillPatternStyle = 0x10;
                    break;
                case PHPExcel1_Style_Fill::FILL_PATTERN_GRAY125:
                    $blockFillPatternStyle = 0x11;
                    break;
                case PHPExcel1_Style_Fill::FILL_PATTERN_GRAY0625:
                    $blockFillPatternStyle = 0x12;
                    break;
                case PHPExcel1_Style_Fill::FILL_GRADIENT_LINEAR:
                    $blockFillPatternStyle = 0x00;
                    break;    // does not exist in BIFF8
                case PHPExcel1_Style_Fill::FILL_GRADIENT_PATH:
                    $blockFillPatternStyle = 0x00;
                    break;    // does not exist in BIFF8
                default:
                    $blockFillPatternStyle = 0x00;
        ...
        if ($bFormatProt == 1) {
            ...
            if ($conditional->getStyle()->getProtection()->getLocked() == PHPExcel1_Style_Protection::PROTECTION_PROTECTED) {
                $dataBlockProtection = 1;
            }
            if ($conditional->getStyle()->getProtection()->getHidden() == PHPExcel1_Style_Protection::PROTECTION_PROTECTED) {
        ...
        foreach ($this->phpSheet->getConditionalStylesCollection() as $cellCoordinate => $conditionalStyles) {
            foreach ($conditionalStyles as $conditional) {
                if ($conditional->getConditionType() == PHPExcel1_Style_Conditional::CONDITION_EXPRESSION
                    ...
                    // Cells
                    $arrCoord = PHPExcel1_Cell::coordinateFromString($cellCoordinate);
                    if (!is_numeric($arrCoord[0])) {
                        $arrCoord[0] = PHPExcel1_Cell::columnIndexFromString($arrCoord[0]);
                    }
