...
/**
 * PHPExcel1_Writer_Excel5_Workbook
 *
...
// */
class PHPExcel1_Writer_Excel5_Workbook extends PHPExcel1_Writer_Excel5_BIFFwriter
{
    /**
     * Formula parser
     *
    ...
    /**
     * XF Writers
     * @var PHPExcel1_Writer_Excel5_Xf[]
     */
     ...
     * Fonts writers
     *
     * @var PHPExcel1_Writer_Excel5_Font[]
     */
     ...
     * Class constructor
     *
     * @param PHPExcel1    $PHPExcel1        The Workbook
     ...
     */
    public function __construct(PHPExcel1 $PHPExcel1 = null, &$str_total, &$str_unique, &$str_table, &$colors, $parser)
    {
        ...
        $countSheets = $PHPExcel1->getSheetCount();
        for ($i = 0; $i < $countSheets; ++$i) {
            $phpSheet = $PHPExcel1->getSheet($i);
     ...
     *
     * @param PHPExcel1_Style_Font $font
     * @return int Index to FONT record
     */
    public function addFont(PHPExcel1_Style_Font $font)
    {
        ...
        if (isset($this->addedFonts[$fontHashCode])) {
            ...
            $fontWriter = new PHPExcel1_Writer_Excel5_Font($font);
    ...
    public function writeWorkbook($pWorksheetSizes = null)
    {
        ...
        $total_worksheets = $this->PHPExcel1->getSheetCount();
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            $this->writeBoundSheet($this->PHPExcel1->getSheet($i), $this->worksheetOffsets[$i]);
        }
    ...
    private function calcSheetOffsets()
    {
        ...
        $total_worksheets = count($this->PHPExcel1->getAllSheets());
        foreach ($this->PHPExcel1->getWorksheetIterator() as $sheet) {
            $offset += $boundsheet_length + strlen(PHPExcel1_Shared_String::UTF8toBIFF8UnicodeShort($sheet->getTitle()));
        }
    ...
    private function writeExternals()
    {
        ...
        for ($i = 0; $i < $countSheets; ++$i) {
            $this->writeExternalSheet($this->PHPExcel1->getSheet($i)->getTitle());
        }
    }
    ...
    private function writeNames()
    {
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            $sheetSetup = $this->PHPExcel1->getSheet($i)->getPageSetup();
            // Write a Name record if the print area has been defined
            if ($sheetSetup->isPrintAreaSet()) {
                // Print area
                $printArea = PHPExcel1_Cell::splitRange($sheetSetup->getPrintArea());
                $printArea = $printArea[0];
                $printArea[0] = PHPExcel1_Cell::coordinateFromString($printArea[0]);
                $printArea[1] = PHPExcel1_Cell::coordinateFromString($printArea[1]);

                $print_rowmin = $printArea[0][1] - 1;
                $print_rowmax = $printArea[1][1] - 1;
                $print_colmin = PHPExcel1_Cell::columnIndexFromString($printArea[0][0]) - 1;
                $print_colmax = PHPExcel1_Cell::columnIndexFromString($printArea[1][0]) - 1;
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            ...
            if ($sheetSetup->isColumnsToRepeatAtLeftSet() && $sheetSetup->isRowsToRepeatAtTopSet()) {
                ...
                $colmin = PHPExcel1_Cell::columnIndexFromString($repeat[0]) - 1;
                $colmax = PHPExcel1_Cell::columnIndexFromString($repeat[1]) - 1;
    ...
    private function writeAllDefinedNamesBiff8()
    {
        ...
        // Named ranges
        if (count($this->PHPExcel1->getNamedRanges()) > 0) {
            // Loop named ranges
            $namedRanges = $this->PHPExcel1->getNamedRanges();
            foreach ($namedRanges as $namedRange) {
                // Create absolute coordinate
                $range = PHPExcel1_Cell::splitRange($namedRange->getRange());
                for ($i = 0; $i < count($range); $i++) {
                    $range[$i][0] = '\'' . str_replace("'", "''", $namedRange->getWorksheet()->getTitle()) . '\'!' . PHPExcel1_Cell::absoluteCoordinate($range[$i][0]);
                    if (isset($range[$i][1])) {
                        $range[$i][1] = PHPExcel1_Cell::absoluteCoordinate($range[$i][1]);
                    }
                }
                $range = PHPExcel1_Cell::buildRange($range); // e.g. Sheet1!$A$1:$B$2
                ...
                try {
                    ...
                    if ($namedRange->getLocalOnly()) {
                        // local scope
                        $scope = $this->PHPExcel1->getIndex($namedRange->getScope()) + 1;
                    } else {
                        // global scope
                        $scope = 0;
                    }
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            ...
            if ($sheetSetup->isPrintAreaSet()) {
                ...
                for ($j = 0; $j < $countPrintArea; ++$j) {
                    ...
                    $printAreaRect[0] = PHPExcel1_Cell::coordinateFromString($printAreaRect[0]);
                    $printAreaRect[1] = PHPExcel1_Cell::coordinateFromString($printAreaRect[1]);

                    $print_rowmin = $printAreaRect[0][1] - 1;
                    $print_rowmax = $printAreaRect[1][1] - 1;
                    $print_colmin = PHPExcel1_Cell::columnIndexFromString($printAreaRect[0][0]) - 1;
                    $print_colmax = PHPExcel1_Cell::columnIndexFromString($printAreaRect[1][0]) - 1;
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            $sheetAutoFilter = $this->PHPExcel1->getSheet($i)->getAutoFilter();
            $autoFilterRange = $sheetAutoFilter->getRange();
            if (!empty($autoFilterRange)) {
                $rangeBounds = PHPExcel1_Cell::rangeBoundaries($autoFilterRange);
    ...
    private function writeDefinedNameBiff8($name, $formulaData, $sheetIndex = 0, $isBuiltIn = false)
    {
        ...
        $nlen = PHPExcel1_Shared_String::CountCharacters($name);

        // name with stripped length field
        $name = substr(PHPExcel1_Shared_String::UTF8toBIFF8UnicodeLong($name), 2);
    ...
    private function writeWindow1()
    {
        ...
        // not supported by PHPExcel1, so there is only one selected sheet, the active
        $ctabsel = 1;       // Number of workbook tabs selected

        $wTabRatio = 0x0258;    // Tab to scrollbar ratio

        // not supported by PHPExcel1, set to 0
        $itabFirst = 0;     // 1st displayed worksheet
        $itabCur   = $this->PHPExcel1->getActiveSheetIndex();    // Active worksheet
     ...
     *
     * @param PHPExcel1_Worksheet  $sheet Worksheet name
    ...
    private function writeBoundSheet($sheet, $offset)
    {
        ...
        switch ($sheet->getSheetState()) {
            case PHPExcel1_Worksheet::SHEETSTATE_VISIBLE:
                $ss = 0x00;
                break;
            case PHPExcel1_Worksheet::SHEETSTATE_HIDDEN:
                $ss = 0x01;
                break;
            case PHPExcel1_Worksheet::SHEETSTATE_VERYHIDDEN:
                $ss = 0x02;
                break;
            default:
                $ss = 0x00;
        ...
        $data .= PHPExcel1_Shared_String::UTF8toBIFF8UnicodeShort($sheetname);
    ...
    private function writeSupbookInternal()
    {
        ...
        $header    = pack("vv", $record, $length);
        $data      = pack("vv", $this->PHPExcel1->getSheetCount(), 0x0401);
    ...
    private function writeNumberFormat($format, $ifmt)
    {
        ...
        $numberFormatString = PHPExcel1_Shared_String::UTF8toBIFF8UnicodeLong($format);
    ...
    private function writeDateMode()
    {
        ...
        $f1904  = (PHPExcel1_Shared_Date::getExcelCalendar() == PHPExcel1_Shared_Date::CALENDAR_MAC_1904)
            ? 1
