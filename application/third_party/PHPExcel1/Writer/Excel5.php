 ...
 */
class PHPExcel1_Writer_Excel5 extends PHPExcel1_Writer_Abstract implements PHPExcel1_Writer_IWriter
{
    /**
     * PHPExcel1 object
     *
    ...
    /**
     * Create a new PHPExcel1_Writer_Excel5
     *
    ...
    public function __construct(PHPExcel1 $PHPExcel1)
    {
        ...
        $this->parser        = new PHPExcel1_Writer_Excel5_Parser();
    }
    ...
    public function save($pFilename = null)
    {
        ...
        $saveDateReturnType = PHPExcel1_Calculation_Functions::getReturnDateType();
        ...
        $this->writerWorkbook = new PHPExcel1_Writer_Excel5_Workbook($this->PHPExcel1, $this->strTotal, $this->strUnique, $this->strTable, $this->colors, $this->parser);

        // Initialise worksheet writers
        $countSheets = $this->PHPExcel1->getSheetCount();
        for ($i = 0; $i < $countSheets; ++$i) {
            $this->writerWorksheets[$i] = new PHPExcel1_Writer_Excel5_Worksheet($this->strTotal, $this->strUnique, $this->strTable, $this->colors, $this->parser, $this->preCalculateFormulas, $this->PHPExcel1->getSheet($i));
        }
        ...
        $cellXfCollection = $this->PHPExcel1->getCellXfCollection();
        for ($i = 0; $i < 15; ++$i) {
            $this->writerWorkbook->addXfWriter($cellXfCollection[0], true);
        }

        // add all the cell Xfs
        foreach ($this->PHPExcel1->getCellXfCollection() as $style) {
        ...
        $OLE = new PHPExcel1_Shared_OLE_PPS_File(PHPExcel1_Shared_OLE::Asc2Ucs($workbookStreamName));
        ...
        if (isset($this->documentSummaryInformation) && !empty($this->documentSummaryInformation)) {
            $OLE_DocumentSummaryInformation = new PHPExcel1_Shared_OLE_PPS_File(PHPExcel1_Shared_OLE::Asc2Ucs(chr(5) . 'DocumentSummaryInformation'));
        ...
        if (isset($this->summaryInformation) && !empty($this->summaryInformation)) {
            $OLE_SummaryInformation = new PHPExcel1_Shared_OLE_PPS_File(PHPExcel1_Shared_OLE::Asc2Ucs(chr(5) . 'SummaryInformation'));
        ...
        $root = new PHPExcel1_Shared_OLE_PPS_Root(time(), time(), $arrRootData);
        // save the OLE file
        $res = $root->save($pFilename);

        PHPExcel1_Calculation_Functions::setReturnDateType($saveDateReturnType);
     ...
     * @return PHPExcel1_Writer_Excel5
     */
    ...
    private function buildWorksheetEschers()
    {
        ...
        foreach ($this->PHPExcel1->getAllsheets() as $sheet) {
            // sheet index
            ...
            $escher = new PHPExcel1_Shared_Escher();

            // dgContainer
            $dgContainer = new PHPExcel1_Shared_Escher_DgContainer();
            ...
            // spgrContainer
            $spgrContainer = new PHPExcel1_Shared_Escher_DgContainer_SpgrContainer();
            $dgContainer->setSpgrContainer($spgrContainer);

            // add one shape which is the group shape
            $spContainer = new PHPExcel1_Shared_Escher_DgContainer_SpgrContainer_SpContainer();
            ...
            foreach ($sheet->getDrawingCollection() as $drawing) {
                ...
                $twoAnchor = PHPExcel1_Shared_Excel5::oneAnchor2twoAnchor($sheet, $coordinates, $offsetX, $offsetY, $width, $height);
            ...
            if (!empty($filterRange)) {
                $rangeBounds = PHPExcel1_Cell::rangeBoundaries($filterRange);
                ...
                while ($iInc <= $iNumColEnd) {
                    ...
                    $oDrawing  = new PHPExcel1_Worksheet_BaseDrawing();
                    // get the coordinates of drawing
                    $cDrawing   = PHPExcel1_Cell::stringFromColumnIndex($iInc - 1) . $rangeBounds[0][1];
                    ...
                    // set coordinates and offsets, client anchor
                    $endCoordinates = PHPExcel1_Cell::stringFromColumnIndex(PHPExcel1_Cell::stringFromColumnIndex($iInc - 1));
    ...
    private function buildWorkbookEscher()
    {
        ...
        // dggContainer
        $dggContainer = new PHPExcel1_Shared_Escher_DggContainer();
        ...
        foreach ($this->PHPExcel1->getAllsheets() as $sheet) {
            ...
            if (count($sheet->getDrawingCollection()) > 0) {
                ...
                foreach ($sheet->getDrawingCollection() as $drawing) {
                    ...
                    $spId = $sheetCountShapes | ($this->PHPExcel1->getIndex($sheet) + 1) << 10;
        ...
        // bstoreContainer
        $bstoreContainer = new PHPExcel1_Shared_Escher_DggContainer_BstoreContainer();
                ...
                if ($drawing instanceof PHPExcel1_Worksheet_Drawing) {
                    ...
                    switch ($imageFormat) {
                        case 1: // GIF, not supported by BIFF8, we convert to PNG
                            $blipType = PHPExcel1_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
                            ob_start();
                            ...
                            break;
                        case 2: // JPEG
                            $blipType = PHPExcel1_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG;
                        ...
                        case 6: // Windows DIB (BMP), we convert to PNG
                            $blipType = PHPExcel1_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
                            ob_start();
                            imagepng(PHPExcel1_Shared_Drawing::imagecreatefrombmp($filename));
                            $blipData = ob_get_contents();
                            ob_end_clean();
                            break;
                        default:
                    ...
                    $blip = new PHPExcel1_Shared_Escher_DggContainer_BstoreContainer_BSE_Blip();
                    $blip->setData($blipData);

                    $BSE = new PHPExcel1_Shared_Escher_DggContainer_BstoreContainer_BSE();
                    ...
                    switch ($drawing->getRenderingFunction()) {
                        case PHPExcel1_Worksheet_MemoryDrawing::RENDERING_JPEG:
                            $blipType = PHPExcel1_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG;
                            $renderingFunction = 'imagejpeg';
                            break;
                        case PHPExcel1_Worksheet_MemoryDrawing::RENDERING_GIF:
                        case PHPExcel1_Worksheet_MemoryDrawing::RENDERING_PNG:
                        case PHPExcel1_Worksheet_MemoryDrawing::RENDERING_DEFAULT:
                            $blipType = PHPExcel1_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
    ...
    private function writeDocumentSummaryInformation()
    {
        ...
        if ($this->PHPExcel1->getProperties()->getCategory()) {
            $dataProp = $this->PHPExcel1->getProperties()->getCategory();
    ...
    private function writeSummaryInformation()
    {
        ...
        //    Title
        if ($this->PHPExcel1->getProperties()->getTitle()) {
            $dataProp = $this->PHPExcel1->getProperties()->getTitle();
            $dataSection[] = array('summary'=> array('pack' => 'V', 'data' => 0x02),
                                   'offset' => array('pack' => 'V'),
        ...
        //    Subject
        if ($this->PHPExcel1->getProperties()->getSubject()) {
            $dataProp = $this->PHPExcel1->getProperties()->getSubject();
        ...
        //    Author (Creator)
        if ($this->PHPExcel1->getProperties()->getCreator()) {
            $dataProp = $this->PHPExcel1->getProperties()->getCreator();
        ...
        //    Keywords
        if ($this->PHPExcel1->getProperties()->getKeywords()) {
            $dataProp = $this->PHPExcel1->getProperties()->getKeywords();
        ...
        if ($this->PHPExcel1->getProperties()->getDescription()) {
            $dataProp = $this->PHPExcel1->getProperties()->getDescription();
        ...
        if ($this->PHPExcel1->getProperties()->getLastModifiedBy()) {
            $dataProp = $this->PHPExcel1->getProperties()->getLastModifiedBy();
        ...
        //    Created Date/Time
        if ($this->PHPExcel1->getProperties()->getCreated()) {
            $dataProp = $this->PHPExcel1->getProperties()->getCreated();
            $dataSection[] = array('summary'=> array('pack' => 'V', 'data' => 0x0C),
                                   'offset' => array('pack' => 'V'),
                                   ...
                                   'data'    => array('data' => PHPExcel1_Shared_OLE::LocalDate2OLE($dataProp)));
            $dataSection_NumProps++;
        }
        //    Modified Date/Time
        if ($this->PHPExcel1->getProperties()->getModified()) {
            $dataProp = $this->PHPExcel1->getProperties()->getModified();
            $dataSection[] = array('summary'=> array('pack' => 'V', 'data' => 0x0D),
                                   'offset' => array('pack' => 'V'),
                                   'type'     => array('pack' => 'V', 'data' => 0x40), // Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
                                   'data'    => array('data' => PHPExcel1_Shared_OLE::LocalDate2OLE($dataProp)));
