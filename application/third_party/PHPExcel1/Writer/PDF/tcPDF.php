...
$pdfRendererClassFile = PHPExcel1_Settings::getPdfRendererPath() . '/tcpdf.php';
if (file_exists($pdfRendererClassFile)) {
    $k_path_url = PHPExcel1_Settings::getPdfRendererPath();
 ...
 */
class PHPExcel1_Writer_PDF_tcPDF extends PHPExcel1_Writer_PDF_Core implements PHPExcel1_Writer_IWriter
{
    /**
    ...
    public function save($pFilename = null)
    {
        ...
        if (!is_null($this->getOrientation())) {
            $orientation = ($this->getOrientation() == PHPExcel1_Worksheet_PageSetup::ORIENTATION_LANDSCAPE)
                ? 'L'
                : 'P';
        }
        //  Override Paper Size
        ...
        //  Document info
        $pdf->SetTitle($this->PHPExcel1->getProperties()->getTitle());
        $pdf->SetAuthor($this->PHPExcel1->getProperties()->getCreator());
        $pdf->SetSubject($this->PHPExcel1->getProperties()->getSubject());
        $pdf->SetKeywords($this->PHPExcel1->getProperties()->getKeywords());
        $pdf->SetCreator($this->PHPExcel1->getProperties()->getCreator());
