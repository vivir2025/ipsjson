 ...
 *
 * @category   PHPExcel1
 * @package    PHPExcel1_Reader_Excel5
...
// http://sourceforge.net/projects/PHPExcel1reader/
...
class PHPExcel1_Reader_Excel5 extends PHPExcel1_Reader_Abstract implements PHPExcel1_Reader_IReader
{
    // ParseXL definitions
     ...
     *
     * @var PHPExcel1_Reader_Excel5_RC4
     */
    private $rc4Key = null;
    ...
    /**
     * Create a new PHPExcel1_Reader_Excel5 instance
     */
    public function __construct()
    {
        $this->readFilter = new PHPExcel1_Reader_DefaultReadFilter();
    }

    /**
     * Can the current PHPExcel1_Reader_IReader read the file?
     *
     * @param     string         $pFilename
     * @return     boolean
     * @throws PHPExcel1_Reader_Exception
     */
    public function canRead($pFilename)
    {
        // Check if file exists
        if (!file_exists($pFilename)) {
            throw new PHPExcel1_Reader_Exception("Could not open " . $pFilename . " for reading! File does not exist.");
        }

        try {
            // Use ParseXL for the hard work.
            $ole = new PHPExcel1_Shared_OLERead();
    ...
    /**
     * Reads names of the worksheets from a file, without parsing the whole file to a PHPExcel1 object
     *
     * @param     string         $pFilename
     * @throws     PHPExcel1_Reader_Exception
     */
    ...
    public function listWorksheetInfo($pFilename)
    {
        ...
        foreach ($this->sheets as $sheet) {
            ...
            $tmpInfo['lastColumnLetter'] = PHPExcel1_Cell::stringFromColumnIndex($tmpInfo['lastColumnIndex']);
    ...
    public function load($pFilename)
    {
        ...
        // Initialisations
        $this->PHPExcel1 = new PHPExcel1;
        $this->PHPExcel1->removeSheetByIndex(0); // remove 1st sheet
        if (!$this->readDataOnly) {
            $this->PHPExcel1->removeCellStyleXfByIndex(0); // remove the default style
            $this->PHPExcel1->removeCellXfByIndex(0); // remove the default style
        }
        ...
        if (!$this->readDataOnly) {
            foreach ($this->objFonts as $objFont) {
                if (isset($objFont->colorIndex)) {
                    $color = PHPExcel1_Reader_Excel5_Color::map($objFont->colorIndex, $this->palette, $this->version);
                    $objFont->getColor()->setRGB($color['rgb']);
                }
            }

            foreach ($this->PHPExcel1->getCellXfCollection() as $objStyle) {
                // fill start and end color
                $fill = $objStyle->getFill();

                if (isset($fill->startcolorIndex)) {
                    $startColor = PHPExcel1_Reader_Excel5_Color::map($fill->startcolorIndex, $this->palette, $this->version);
                    $fill->getStartColor()->setRGB($startColor['rgb']);
                }
                if (isset($fill->endcolorIndex)) {
                    $endColor = PHPExcel1_Reader_Excel5_Color::map($fill->endcolorIndex, $this->palette, $this->version);
                ...
                if (isset($top->colorIndex)) {
                    $borderTopColor = PHPExcel1_Reader_Excel5_Color::map($top->colorIndex, $this->palette, $this->version);
                    $top->getColor()->setRGB($borderTopColor['rgb']);
                }
                if (isset($right->colorIndex)) {
                    $borderRightColor = PHPExcel1_Reader_Excel5_Color::map($right->colorIndex, $this->palette, $this->version);
                    $right->getColor()->setRGB($borderRightColor['rgb']);
                }
                if (isset($bottom->colorIndex)) {
                    $borderBottomColor = PHPExcel1_Reader_Excel5_Color::map($bottom->colorIndex, $this->palette, $this->version);
                    $bottom->getColor()->setRGB($borderBottomColor['rgb']);
                }
                if (isset($left->colorIndex)) {
                    $borderLeftColor = PHPExcel1_Reader_Excel5_Color::map($left->colorIndex, $this->palette, $this->version);
                    $left->getColor()->setRGB($borderLeftColor['rgb']);
                }
                if (isset($diagonal->colorIndex)) {
                    $borderDiagonalColor = PHPExcel1_Reader_Excel5_Color::map($diagonal->colorIndex, $this->palette, $this->version);
        ...
        if (!$this->readDataOnly && $this->drawingGroupData) {
            $escherWorkbook = new PHPExcel1_Shared_Escher();
            $reader = new PHPExcel1_Reader_Excel5_Escher($escherWorkbook);
            $escherWorkbook = $reader->load($this->drawingGroupData);

            // debug Escher stream
            //$debug = new Debug_Escher(new PHPExcel1_Shared_Escher());
        ...
        foreach ($this->sheets as $sheet) {
            ...
            // add sheet to PHPExcel1 object
            $this->phpSheet = $this->PHPExcel1->createSheet();
            ...
            if (!$this->readDataOnly && $this->drawingData) {
                $escherWorksheet = new PHPExcel1_Shared_Escher();
                $reader = new PHPExcel1_Reader_Excel5_Escher($escherWorksheet);
...
//                echo '<br />';
                ...
                if (isset($allSpContainers[$n + 1]) && is_object($allSpContainers[$n + 1])) {
                    ...
                    list($startColumn, $startRow) = PHPExcel1_Cell::coordinateFromString($spContainer->getStartCoordinates());
                    list($endColumn, $endRow) = PHPExcel1_Cell::coordinateFromString($spContainer->getEndCoordinates());
                    ...
                    $width = PHPExcel1_Shared_Excel5::getDistanceX($this->phpSheet, $startColumn, $startOffsetX, $endColumn, $endOffsetX);
                    $height = PHPExcel1_Shared_Excel5::getDistanceY($this->phpSheet, $startRow, $startOffsetY, $endRow, $endOffsetY);

                    // calculate offsetX and offsetY of the shape
                    $offsetX = $startOffsetX * PHPExcel1_Shared_Excel5::sizeCol($this->phpSheet, $startColumn) / 1024;
                    $offsetY = $startOffsetY * PHPExcel1_Shared_Excel5::sizeRow($this->phpSheet, $startRow) / 256;
...
//                            echo 'Picture Object<br />';
                            ...
                            if ($blip = $BSE->getBlip()) {
                                ...
                                $drawing = new PHPExcel1_Worksheet_MemoryDrawing();
                                ...
                                switch ($blipType) {
                                    case PHPExcel1_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG:
                                        $drawing->setRenderingFunction(PHPExcel1_Worksheet_MemoryDrawing::RENDERING_JPEG);
                                        $drawing->setMimeType(PHPExcel1_Worksheet_MemoryDrawing::MIMETYPE_JPEG);
                                        break;
                                    case PHPExcel1_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG:
                                        $drawing->setRenderingFunction(PHPExcel1_Worksheet_MemoryDrawing::RENDERING_PNG);
                                        $drawing->setMimeType(PHPExcel1_Worksheet_MemoryDrawing::MIMETYPE_PNG);
                                        break;
                                }
            ...
            if ($this->version == self::XLS_BIFF8) {
                foreach ($this->sharedFormulaParts as $cell => $baseCell) {
                    list($column, $row) = PHPExcel1_Cell::coordinateFromString($cell);
                    if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($column, $row, $this->phpSheet->getTitle())) {
                        $formula = $this->getFormulaFromStructure($this->sharedFormulas[$baseCell], $cell);
                        $this->phpSheet->getCell($cell)->setValueExplicit('=' . $formula, PHPExcel1_Cell_DataType::TYPE_FORMULA);
                    }
                }
            }
...
//                    echo '<br />';
            ...
            if ($definedName['isBuiltInName']) {
                switch ($definedName['name']) {
                    case pack('C', 0x06):
                        ...
                        if ($docSheet = $this->PHPExcel1->getSheetByName($sheetName)) {
                    ...
                    case pack('C', 0x07):
                        ...
                        foreach ($ranges as $range) {
                            ...
                            if (count($explodes) == 2) {
                                if ($docSheet = $this->PHPExcel1->getSheetByName($explodes[0])) {
                                    $extractedRange = $explodes[1];
                                    $extractedRange = str_replace('$', '', $extractedRange);

                                    $coordinateStrings = explode(':', $extractedRange);
                                    if (count($coordinateStrings) == 2) {
                                        list($firstColumn, $firstRow) = PHPExcel1_Cell::coordinateFromString($coordinateStrings[0]);
                                        list($lastColumn, $lastRow) = PHPExcel1_Cell::coordinateFromString($coordinateStrings[1]);
                ...
                if (count($explodes) == 2) {
                    if (($docSheet = $this->PHPExcel1->getSheetByName($explodes[0])) ||
                        ($docSheet = $this->PHPExcel1->getSheetByName(trim($explodes[0], "'")))) {
                        $extractedRange = $explodes[1];
                        $extractedRange = str_replace('$', '', $extractedRange);

                        $localOnly = ($definedName['scope'] == 0) ? false : true;

                        $scope = ($definedName['scope'] == 0) ? null : $this->PHPExcel1->getSheetByName($this->sheets[$definedName['scope'] - 1]['name']);

                        $this->PHPExcel1->addNamedRange(new PHPExcel1_NamedRange((string)$definedName['name'], $docSheet, $extractedRange, $localOnly, $scope));
                    }
                } else {
                    //    Named Value
    ...
    private function readRecordData($data, $pos, $len)
    {
        ...
        if ($this->encryption == self::MS_BIFF_CRYPTO_RC4) {
            ...
            throw new PHPExcel1_Reader_Exception('XOr encryption not supported');
        }
        return $recordData;
    }
        ...
        for ($i = 0; $i < $countProperties; ++$i) {
            ...
            switch ($type) {
                ...
                case 0x1E: // null-terminated string prepended by dword string length
                    ...
                    $value = PHPExcel1_Shared_String::ConvertEncoding($value, 'UTF-8', $codePage);
                    $value = rtrim($value);
                    break;
                case 0x40: // Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
                    // PHP-time
                    $value = PHPExcel1_Shared_OLE::OLE2LocalDate(substr($this->summaryInformation, $secOffset + 4 + $offset, 8));
                    break;
            ...
            switch ($id) {
                case 0x01:    //    Code Page
                    $codePage = PHPExcel1_Shared_CodePage::NumberToName($value);
                    break;
                case 0x02:    //    Title
                    $this->PHPExcel1->getProperties()->setTitle($value);
                    break;
                case 0x03:    //    Subject
                    $this->PHPExcel1->getProperties()->setSubject($value);
                    break;
                case 0x04:    //    Author (Creator)
                    $this->PHPExcel1->getProperties()->setCreator($value);
                    break;
                case 0x05:    //    Keywords
                    $this->PHPExcel1->getProperties()->setKeywords($value);
                    break;
                case 0x06:    //    Comments (Description)
                    $this->PHPExcel1->getProperties()->setDescription($value);
                    break;
                case 0x07:    //    Template
                    //    Not supported by PHPExcel1
                    break;
                case 0x08:    //    Last Saved By (LastModifiedBy)
                    $this->PHPExcel1->getProperties()->setLastModifiedBy($value);
                    break;
                case 0x09:    //    Revision
                    //    Not supported by PHPExcel1
                    break;
                ...
                case 0x0C:    //    Created Date/Time
                    $this->PHPExcel1->getProperties()->setCreated($value);
                    break;
                case 0x0D:    //    Modified Date/Time
                    $this->PHPExcel1->getProperties()->setModified($value);
                    break;
...
//            echo 'Type is ', $type,', ';
            ...
            switch ($type) {
                ...
                case 0x40:    //    Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
                    // PHP-Time
                    $value = PHPExcel1_Shared_OLE::OLE2LocalDate(substr($this->documentSummaryInformation, $secOffset + 4 + $offset, 8));
                    break;
                case 0x47:    //    Clipboard format
                    // not needed yet, fix later if necessary
                    break;
            ...
            switch ($id) {
                case 0x01:    //    Code Page
                    ...
                    break;
                case 0x02:    //    Category
                    $this->PHPExcel1->getProperties()->setCategory($value);
                    break;
                ...
                case 0x0D:    //    Titles of Parts
                    ...
                    break;
                case 0x0E:    //    Manager
                    $this->PHPExcel1->getProperties()->setManager($value);
                    break;
                case 0x0F:    //    Company
                    $this->PHPExcel1->getProperties()->setCompany($value);
                    break;
...
//        echo '<br />';
        ...
        switch ($substreamType) {
            case self::XLS_WorkbookGlobals:
                ...
                if (($version != self::XLS_BIFF8) && ($version != self::XLS_BIFF7)) {
                    throw new PHPExcel1_Reader_Exception('Cannot read this Excel file. Version is too old.');
                }
    ...
    private function readFilepass()
    {
        ...
        if ($length != 54) {
            throw new PHPExcel1_Reader_Exception('Unexpected file pass record length');
        }
        ...
        if (!$this->verifyPassword('VelvetSweatshop', substr($recordData, 6, 16), substr($recordData, 22, 16), substr($recordData, 38, 16), $this->md5Ctxt)) {
            throw new PHPExcel1_Reader_Exception('Decryption password incorrect');
        }
     ...
     *
     * @return PHPExcel1_Reader_Excel5_RC4
     */
    private function makeKey($block, $valContext)
    {
        ...
        $md5 = new PHPExcel1_Reader_Excel5_MD5();
        $md5->add($pwarray);

        $s = $md5->getContext();
        return new PHPExcel1_Reader_Excel5_RC4($s);
    }
    ...
    private function readCodepage()
    {
        ...
        $this->codepage = PHPExcel1_Shared_CodePage::NumberToName($codepage);
    }
    ...
    private function readDateMode()
    {
        ...
        PHPExcel1_Shared_Date::setExcelCalendar(PHPExcel1_Shared_Date::CALENDAR_WINDOWS_1900);
        if (ord($recordData{0}) == 1) {
            PHPExcel1_Shared_Date::setExcelCalendar(PHPExcel1_Shared_Date::CALENDAR_MAC_1904);
        }
    }
    ...
    private function readFont()
    {
        ...
        if (!$this->readDataOnly) {
            $objFont = new PHPExcel1_Style_Font();
            ...
            switch ($underlineType) {
                case 0x00:
                    break; // no underline
                case 0x01:
                    $objFont->setUnderline(PHPExcel1_Style_Font::UNDERLINE_SINGLE);
                    break;
                case 0x02:
                    $objFont->setUnderline(PHPExcel1_Style_Font::UNDERLINE_DOUBLE);
                    break;
                case 0x21:
                    $objFont->setUnderline(PHPExcel1_Style_Font::UNDERLINE_SINGLEACCOUNTING);
                    break;
                case 0x22:
                    $objFont->setUnderline(PHPExcel1_Style_Font::UNDERLINE_DOUBLEACCOUNTING);
                    break;
            }
    ...
    private function readXf()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            } elseif (($code = PHPExcel1_Style_NumberFormat::builtInFormatCode($numberFormatIndex)) !== '') {
            ...
            $objStyle->getProtection()->setLocked($isLocked ? PHPExcel1_Style_Protection::PROTECTION_INHERIT : PHPExcel1_Style_Protection::PROTECTION_UNPROTECTED);

            // bit 1; mask 0x02; 1 = Formula is hidden
            $isHidden = (0x02 & $xfTypeProt) >> 1;
            $objStyle->getProtection()->setHidden($isHidden ? PHPExcel1_Style_Protection::PROTECTION_PROTECTED : PHPExcel1_Style_Protection::PROTECTION_UNPROTECTED);
            ...
            switch ($horAlign) {
                case 0:
                    $objStyle->getAlignment()->setHorizontal(PHPExcel1_Style_Alignment::HORIZONTAL_GENERAL);
                    break;
                case 1:
                    $objStyle->getAlignment()->setHorizontal(PHPExcel1_Style_Alignment::HORIZONTAL_LEFT);
                    break;
                case 2:
                    $objStyle->getAlignment()->setHorizontal(PHPExcel1_Style_Alignment::HORIZONTAL_CENTER);
                    break;
                case 3:
                    $objStyle->getAlignment()->setHorizontal(PHPExcel1_Style_Alignment::HORIZONTAL_RIGHT);
                    break;
                case 4:
                    $objStyle->getAlignment()->setHorizontal(PHPExcel1_Style_Alignment::HORIZONTAL_FILL);
                    break;
                case 5:
                    $objStyle->getAlignment()->setHorizontal(PHPExcel1_Style_Alignment::HORIZONTAL_JUSTIFY);
                    break;
                case 6:
                    $objStyle->getAlignment()->setHorizontal(PHPExcel1_Style_Alignment::HORIZONTAL_CENTER_CONTINUOUS);
                    break;
            }
            ...
            switch ($vertAlign) {
                case 0:
                    $objStyle->getAlignment()->setVertical(PHPExcel1_Style_Alignment::VERTICAL_TOP);
                    break;
                case 1:
                    $objStyle->getAlignment()->setVertical(PHPExcel1_Style_Alignment::VERTICAL_CENTER);
                    break;
                case 2:
                    $objStyle->getAlignment()->setVertical(PHPExcel1_Style_Alignment::VERTICAL_BOTTOM);
                    break;
                case 3:
                    $objStyle->getAlignment()->setVertical(PHPExcel1_Style_Alignment::VERTICAL_JUSTIFY);
                    break;
            }
            ...
            if ($this->version == self::XLS_BIFF8) {
                ...
                if ($bordersLeftStyle = PHPExcel1_Reader_Excel5_Style_Border::lookup((0x0000000F & self::getInt4d($recordData, 10)) >> 0)) {
                    $objStyle->getBorders()->getLeft()->setBorderStyle($bordersLeftStyle);
                }
                // bit: 7-4; mask: 0x000000F0; right style
                if ($bordersRightStyle = PHPExcel1_Reader_Excel5_Style_Border::lookup((0x000000F0 & self::getInt4d($recordData, 10)) >> 4)) {
                    $objStyle->getBorders()->getRight()->setBorderStyle($bordersRightStyle);
                }
                // bit: 11-8; mask: 0x00000F00; top style
                if ($bordersTopStyle = PHPExcel1_Reader_Excel5_Style_Border::lookup((0x00000F00 & self::getInt4d($recordData, 10)) >> 8)) {
                    $objStyle->getBorders()->getTop()->setBorderStyle($bordersTopStyle);
                }
                // bit: 15-12; mask: 0x0000F000; bottom style
                if ($bordersBottomStyle = PHPExcel1_Reader_Excel5_Style_Border::lookup((0x0000F000 & self::getInt4d($recordData, 10)) >> 12)) {
                ...
                if ($diagonalUp == false && $diagonalDown == false) {
                    $objStyle->getBorders()->setDiagonalDirection(PHPExcel1_Style_Borders::DIAGONAL_NONE);
                } elseif ($diagonalUp == true && $diagonalDown == false) {
                    $objStyle->getBorders()->setDiagonalDirection(PHPExcel1_Style_Borders::DIAGONAL_UP);
                } elseif ($diagonalUp == false && $diagonalDown == true) {
                    $objStyle->getBorders()->setDiagonalDirection(PHPExcel1_Style_Borders::DIAGONAL_DOWN);
                } elseif ($diagonalUp == true && $diagonalDown == true) {
                    $objStyle->getBorders()->setDiagonalDirection(PHPExcel1_Style_Borders::DIAGONAL_BOTH);
                }
                ...
                if ($bordersDiagonalStyle = PHPExcel1_Reader_Excel5_Style_Border::lookup((0x01E00000 & self::getInt4d($recordData, 14)) >> 21)) {
                    $objStyle->getBorders()->getDiagonal()->setBorderStyle($bordersDiagonalStyle);
                }

                // bit: 31-26; mask: 0xFC000000 fill pattern
                if ($fillType = PHPExcel1_Reader_Excel5_Style_FillPattern::lookup((0xFC000000 & self::getInt4d($recordData, 14)) >> 26)) {
                ...
                $objStyle->getFill()->setFillType(PHPExcel1_Reader_Excel5_Style_FillPattern::lookup((0x003F0000 & $borderAndBackground) >> 16));

                // bit: 24-22; mask: 0x01C00000; bottom line style
                $objStyle->getBorders()->getBottom()->setBorderStyle(PHPExcel1_Reader_Excel5_Style_Border::lookup((0x01C00000 & $borderAndBackground) >> 22));
                ...
                $objStyle->getBorders()->getTop()->setBorderStyle(PHPExcel1_Reader_Excel5_Style_Border::lookup((0x00000007 & $borderLines) >> 0));

                // bit: 5-3; mask: 0x00000038; left line style
                $objStyle->getBorders()->getLeft()->setBorderStyle(PHPExcel1_Reader_Excel5_Style_Border::lookup((0x00000038 & $borderLines) >> 3));

                // bit: 8-6; mask: 0x000001C0; right line style
                $objStyle->getBorders()->getRight()->setBorderStyle(PHPExcel1_Reader_Excel5_Style_Border::lookup((0x000001C0 & $borderLines) >> 6));
            ...
            if ($isCellStyleXf) {
                ...
                if ($this->xfIndex == 0) {
                    $this->PHPExcel1->addCellStyleXf($objStyle);
                    $this->mapCellStyleXfIndex[$this->xfIndex] = 0;
                }
            } else {
                // we read all cell XF records
                $this->PHPExcel1->addCellXf($objStyle);
                $this->mapCellXfIndex[$this->xfIndex] = count($this->PHPExcel1->getCellXfCollection()) - 1;
            }
                ...
                switch ($extType) {
                    case 4:        // fill start color
                        ...
                        if ($xclfType == 2) {
                            ...
                            if (isset($this->mapCellXfIndex[$ixfe])) {
                                $fill = $this->PHPExcel1->getCellXfByIndex($this->mapCellXfIndex[$ixfe])->getFill();
                    ...
                    case 7:        // border color top
                        ...
                        if ($xclfType == 2) {
                            ...
                            // modify the relevant style property
                            if (isset($this->mapCellXfIndex[$ixfe])) {
                                $top = $this->PHPExcel1->getCellXfByIndex($this->mapCellXfIndex[$ixfe])->getBorders()->getTop();
                    ...
                    case 8:        // border color bottom
                        ...
                        if ($xclfType == 2) {
                            ...
                            // modify the relevant style property
                            if (isset($this->mapCellXfIndex[$ixfe])) {
                                $bottom = $this->PHPExcel1->getCellXfByIndex($this->mapCellXfIndex[$ixfe])->getBorders()->getBottom();
                    ...
                    case 9:        // border color left
                        ...
                        if ($xclfType == 2) {
                            ...
                            // modify the relevant style property
                            if (isset($this->mapCellXfIndex[$ixfe])) {
                                $left = $this->PHPExcel1->getCellXfByIndex($this->mapCellXfIndex[$ixfe])->getBorders()->getLeft();
                    ...
                    case 10:        // border color right
                        ...
                        if ($xclfType == 2) {
                            ...
                            // modify the relevant style property
                            if (isset($this->mapCellXfIndex[$ixfe])) {
                                $right = $this->PHPExcel1->getCellXfByIndex($this->mapCellXfIndex[$ixfe])->getBorders()->getRight();
                    ...
                    case 11:        // border color diagonal
                        ...
                        if ($xclfType == 2) {
                            ...
                            // modify the relevant style property
                            if (isset($this->mapCellXfIndex[$ixfe])) {
                                $diagonal = $this->PHPExcel1->getCellXfByIndex($this->mapCellXfIndex[$ixfe])->getBorders()->getDiagonal();
                    ...
                    case 13:    // font color
                        ...
                        if ($xclfType == 2) {
                            ...
                            // modify the relevant style property
                            if (isset($this->mapCellXfIndex[$ixfe])) {
                                $font = $this->PHPExcel1->getCellXfByIndex($this->mapCellXfIndex[$ixfe])->getFont();
    ...
    private function readStyle()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            if ($isBuiltIn) {
                ...
                // user-defined; not supported by PHPExcel1
            }
        }
    }
    ...
    private function readSheet()
    {
        ...
        switch (ord($recordData{4})) {
            case 0x00:
                $sheetState = PHPExcel1_Worksheet::SHEETSTATE_VISIBLE;
                break;
            case 0x01:
                $sheetState = PHPExcel1_Worksheet::SHEETSTATE_HIDDEN;
                break;
            case 0x02:
                $sheetState = PHPExcel1_Worksheet::SHEETSTATE_VERYHIDDEN;
                break;
            default:
                $sheetState = PHPExcel1_Worksheet::SHEETSTATE_VISIBLE;
                break;
    ...
    private function readHorizontalPageBreaks()
    {
        ...
        if ($this->version == self::XLS_BIFF8 && !$this->readDataOnly) {
            ...
            for ($i = 0; $i < $nm; ++$i) {
                ...
                $this->phpSheet->setBreakByColumnAndRow($cf, $r, PHPExcel1_Worksheet::BREAK_ROW);
            }
        }
    }
    ...
    private function readVerticalPageBreaks()
    {
        ...
        if ($this->version == self::XLS_BIFF8 && !$this->readDataOnly) {
            ...
            for ($i = 0; $i < $nm; ++$i) {
                ...
                $this->phpSheet->setBreakByColumnAndRow($c, $rf, PHPExcel1_Worksheet::BREAK_COLUMN);
            }
        }
    }
    ...
    private function readPageSetup()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            if (!$isNotInit) {
                ...
                switch ($isPortrait) {
                    case 0:
                        $this->phpSheet->getPageSetup()->setOrientation(PHPExcel1_Worksheet_PageSetup::ORIENTATION_LANDSCAPE);
                        break;
                    case 1:
                        $this->phpSheet->getPageSetup()->setOrientation(PHPExcel1_Worksheet_PageSetup::ORIENTATION_PORTRAIT);
                        break;
                }
    ...
    private function readRk()
    {
        ...
        $columnString = PHPExcel1_Cell::stringFromColumnIndex($column);
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            // add cell
            $cell->setValueExplicit($numValue, PHPExcel1_Cell_DataType::TYPE_NUMERIC);
        }
    }
    ...
    private function readLabelSst()
    {
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            if (($fmtRuns = $this->sst[$index]['fmtRuns']) && !$this->readDataOnly) {
                ...
                $richText = new PHPExcel1_RichText();
                $charPos = 0;
                $sstCount = count($this->sst[$index]['fmtRuns']);
                for ($i = 0; $i <= $sstCount; ++$i) {
                    if (isset($fmtRuns[$i])) {
                        $text = PHPExcel1_Shared_String::Substring($this->sst[$index]['value'], $charPos, $fmtRuns[$i]['charPos'] - $charPos);
                        $charPos = $fmtRuns[$i]['charPos'];
                    } else {
                        $text = PHPExcel1_Shared_String::Substring($this->sst[$index]['value'], $charPos, PHPExcel1_Shared_String::CountCharacters($this->sst[$index]['value']));
                    }

                    if (PHPExcel1_Shared_String::CountCharacters($text) > 0) {
                ...
                if ($this->readEmptyCells || trim($richText->getPlainText()) !== '') {
                    $cell = $this->phpSheet->getCell($columnString . ($row + 1));
                    $cell->setValueExplicit($richText, PHPExcel1_Cell_DataType::TYPE_STRING);
                    $emptyCell = false;
                }
            } else {
                if ($this->readEmptyCells || trim($this->sst[$index]['value']) !== '') {
                    $cell = $this->phpSheet->getCell($columnString . ($row + 1));
                    $cell->setValueExplicit($this->sst[$index]['value'], PHPExcel1_Cell_DataType::TYPE_STRING);
                    $emptyCell = false;
                }
            }
    ...
    private function readMulRk()
    {
        ...
        for ($i = 0; $i < $columns; ++$i) {
            $columnString = PHPExcel1_Cell::stringFromColumnIndex($colFirst + $i);
    ...
    private function readFormula()
    {
        ...
        if ($isPartOfSharedFormula) {
            ...
            $this->_baseCell = PHPExcel1_Cell::stringFromColumnIndex($baseCol). ($baseRow + 1);
        }
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            if ((ord($recordData{6}) == 0) && (ord($recordData{12}) == 255) && (ord($recordData{13}) == 255)) {
                ...
                $dataType = PHPExcel1_Cell_DataType::TYPE_STRING;
                ...
                $dataType = PHPExcel1_Cell_DataType::TYPE_BOOL;
                $value = (bool) ord($recordData{8});
            } elseif ((ord($recordData{6}) == 2)
                && (ord($recordData{12}) == 255)
                && (ord($recordData{13}) == 255)) {
                // Error formula. Error code is in +2
                $dataType = PHPExcel1_Cell_DataType::TYPE_ERROR;
                $value = PHPExcel1_Reader_Excel5_ErrorCode::lookup(ord($recordData{8}));
            } elseif ((ord($recordData{6}) == 3)
                && (ord($recordData{12}) == 255)
                && (ord($recordData{13}) == 255)) {
                // Formula result is a null string
                $dataType = PHPExcel1_Cell_DataType::TYPE_NULL;
                $value = '';
            } else {
                // forumla result is a number, first 14 bytes like _NUMBER record
                $dataType = PHPExcel1_Cell_DataType::TYPE_NUMERIC;
            ...
            if (!$isPartOfSharedFormula) {
                ...
                try {
                    if ($this->version != self::XLS_BIFF8) {
                        throw new PHPExcel1_Reader_Exception('Not BIFF8. Can only read BIFF8 formulas');
                    }
                    $formula = $this->getFormulaFromStructure($formulaStructure); // get formula in human language
                    $cell->setValueExplicit('=' . $formula, PHPExcel1_Cell_DataType::TYPE_FORMULA);
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            switch ($isError) {
                case 0: // boolean
                    ...
                    // add cell value
                    $cell->setValueExplicit($value, PHPExcel1_Cell_DataType::TYPE_BOOL);
                    break;
                case 1: // error type
                    $value = PHPExcel1_Reader_Excel5_ErrorCode::lookup($boolErr);

                    // add cell value
                    $cell->setValueExplicit($value, PHPExcel1_Cell_DataType::TYPE_ERROR);
                    break;
            }
    ...
    private function readMulBlank()
    {
        ...
        if (!$this->readDataOnly && $this->readEmptyCells) {
            for ($i = 0; $i < $length / 2 - 3; ++$i) {
                $columnString = PHPExcel1_Cell::stringFromColumnIndex($fc + $i);
    ...
    private function readLabel()
    {
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            if ($this->readEmptyCells || trim($value) !== '') {
                $cell = $this->phpSheet->getCell($columnString . ($row + 1));
                $cell->setValueExplicit($value, PHPExcel1_Cell_DataType::TYPE_STRING);
    ...
    private function readBlank()
    {
        ...
        $columnString = PHPExcel1_Cell::stringFromColumnIndex($col);
...
//        echo '<br />';
    ...
    private function readWindow2()
    {
        ...
        if ($isActive) {
            $this->PHPExcel1->setActiveSheetIndex($this->PHPExcel1->getIndex($this->phpSheet));
        }
        ...
        if ($this->phpSheet->getSheetView()->getView() !== PHPExcel1_Worksheet_SheetView::SHEETVIEW_PAGE_LAYOUT) {
            //NOTE: this setting is inferior to page layout view(Excel2007-)
            $view = $isPageBreakPreview ? PHPExcel1_Worksheet_SheetView::SHEETVIEW_PAGE_BREAK_PREVIEW : PHPExcel1_Worksheet_SheetView::SHEETVIEW_NORMAL;
    ...
    private function readPageLayoutView()
    {
        ...
        if ($fPageLayoutView === 1) {
            $this->phpSheet->getSheetView()->setView(PHPExcel1_Worksheet_SheetView::SHEETVIEW_PAGE_LAYOUT);
    ...
    private function readPane()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            if ($this->frozen) {
                // frozen panes
                $this->phpSheet->freezePane(PHPExcel1_Cell::stringFromColumnIndex($px) . ($py + 1));
            } else {
                // unfrozen panes; split windows; not supported by PHPExcel1 core
            }
        }
    }
    ...
    private function includeCellRangeFiltered($cellRangeAddress)
    {
        ...
        if ($this->getReadFilter() !== null) {
            ...
            $rangeBoundaries = PHPExcel1_Cell::getRangeBoundaries($cellRangeAddress);
            $rangeBoundaries[1][0]++;
    ...
    private function readHyperLink()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            foreach (PHPExcel1_Cell::extractAllCellReferencesInRange($cellRange) as $coordinate) {
    ...
    private function readDataValidation()
    {
        ...
        switch ($type) {
            case 0x00:
                $type = PHPExcel1_Cell_DataValidation::TYPE_NONE;
                break;
            case 0x01:
                $type = PHPExcel1_Cell_DataValidation::TYPE_WHOLE;
                break;
            case 0x02:
                $type = PHPExcel1_Cell_DataValidation::TYPE_DECIMAL;
                break;
            case 0x03:
                $type = PHPExcel1_Cell_DataValidation::TYPE_LIST;
                break;
            case 0x04:
                $type = PHPExcel1_Cell_DataValidation::TYPE_DATE;
                break;
            case 0x05:
                $type = PHPExcel1_Cell_DataValidation::TYPE_TIME;
                break;
            case 0x06:
                $type = PHPExcel1_Cell_DataValidation::TYPE_TEXTLENGTH;
                break;
            case 0x07:
                $type = PHPExcel1_Cell_DataValidation::TYPE_CUSTOM;
                break;
        }
        ...
        switch ($errorStyle) {
            case 0x00:
                $errorStyle = PHPExcel1_Cell_DataValidation::STYLE_STOP;
                break;
            case 0x01:
                $errorStyle = PHPExcel1_Cell_DataValidation::STYLE_WARNING;
                break;
            case 0x02:
                $errorStyle = PHPExcel1_Cell_DataValidation::STYLE_INFORMATION;
                break;
        }
        ...
        switch ($operator) {
            case 0x00:
                $operator = PHPExcel1_Cell_DataValidation::OPERATOR_BETWEEN;
                break;
            case 0x01:
                $operator = PHPExcel1_Cell_DataValidation::OPERATOR_NOTBETWEEN;
                break;
            case 0x02:
                $operator = PHPExcel1_Cell_DataValidation::OPERATOR_EQUAL;
                break;
            case 0x03:
                $operator = PHPExcel1_Cell_DataValidation::OPERATOR_NOTEQUAL;
                break;
            case 0x04:
                $operator = PHPExcel1_Cell_DataValidation::OPERATOR_GREATERTHAN;
                break;
            case 0x05:
                $operator = PHPExcel1_Cell_DataValidation::OPERATOR_LESSTHAN;
                break;
            case 0x06:
                $operator = PHPExcel1_Cell_DataValidation::OPERATOR_GREATERTHANOREQUAL;
                break;
            case 0x07:
                $operator = PHPExcel1_Cell_DataValidation::OPERATOR_LESSTHANOREQUAL;
                break;
        }
        ...
        try {
            ...
            if ($type == PHPExcel1_Cell_DataValidation::TYPE_LIST) {
        ...
        foreach ($cellRangeAddresses as $cellRange) {
            ...
            foreach (PHPExcel1_Cell::extractAllCellReferencesInRange($stRange) as $coordinate) {
        ...
        if (!$this->readDataOnly) {
            ...
            switch ($sz) {
                case 0x14:
                    ...
                    $color = PHPExcel1_Reader_Excel5_Color::map($colorIndex, $this->palette, $this->version);
    ...
    //            var_dump($bcBitCount);
                ...
                $drawing = new PHPExcel1_Worksheet_Drawing();
    ...
    private function getNextToken($formulaData, $baseCell = 'A1')
    {
        ...
        switch ($id) {
                    ...
                    case 0x41:
                        ...
                        switch (ord($formulaData[2])) {
                            ...
                            case 0x05:
                                ...
                                break;
                            default:
                                throw new PHPExcel1_Reader_Exception('Unrecognized space type in tAttrSpace token');
                                break;
                        }
                        ...
                        break;
                    default:
                        throw new PHPExcel1_Reader_Exception('Unrecognized attribute flag in tAttr token');
                        break;
                }
                break;
            case 0x1C:    //    error code
                ...
                $name = 'tErr';
                $size = 2;
                $data = PHPExcel1_Reader_Excel5_ErrorCode::lookup(ord($formulaData[1]));
                break;
            case 0x1D:    //    boolean
            ...
            case 0x61:
                ...
                switch (self::getInt2d($formulaData, 1)) {
                    ...
                    case 368:
                        ...
                        break;
                    default:
                        throw new PHPExcel1_Reader_Exception('Unrecognized function in formula');
                        break;
                }
            ...
            default:
                throw new PHPExcel1_Reader_Exception('Unrecognized token ' . sprintf('%02X', $id) . ' in formula');
                break;
        }
    ...
    private function readBIFF8CellAddress($cellAddressStructure)
    {
        ...
        $column = PHPExcel1_Cell::stringFromColumnIndex(0x00FF & self::getInt2d($cellAddressStructure, 2));
    ...
    {
        list($baseCol, $baseRow) = PHPExcel1_Cell::coordinateFromString($baseCell);
        $baseCol = PHPExcel1_Cell::columnIndexFromString($baseCol) - 1;
        ...
        // bit: 14; mask 0x4000; (1 = relative column index, 0 = absolute column index)
        if (!(0x4000 & self::getInt2d($cellAddressStructure, 2))) {
            $column = PHPExcel1_Cell::stringFromColumnIndex($colIndex);
            $column = '$' . $column;
        } else {
            $colIndex = ($colIndex <= 127) ? $colIndex : $colIndex - 256;
            $column = PHPExcel1_Cell::stringFromColumnIndex($baseCol + $colIndex);
        }
    ...
    private function readBIFF5CellRangeAddressFixed($subData)
    {
        ...
        // check values
        if ($fr > $lr || $fc > $lc) {
            throw new PHPExcel1_Reader_Exception('Not a cell range address');
        }

        // column index to letter
        $fc = PHPExcel1_Cell::stringFromColumnIndex($fc);
        $lc = PHPExcel1_Cell::stringFromColumnIndex($lc);
    ...
    private function readBIFF8CellRangeAddress($subData)
    {
        ...
        // bit: 7-0; mask 0x00FF; column index
        $fc = PHPExcel1_Cell::stringFromColumnIndex(0x00FF & self::getInt2d($subData, 4));
        ...
        // bit: 7-0; mask 0x00FF; column index
        $lc = PHPExcel1_Cell::stringFromColumnIndex(0x00FF & self::getInt2d($subData, 6));
    ...
    private function readBIFF8CellRangeAddressB($subData, $baseCell = 'A1')
    {
        ...
        if (!(0x4000 & self::getInt2d($subData, 4))) {
            ...
            $fc = PHPExcel1_Cell::stringFromColumnIndex($fcIndex);
            $fc = '$' . $fc;
        } else {
            // column offset
            $fcIndex = ($fcIndex <= 127) ? $fcIndex : $fcIndex - 256;
            $fc = PHPExcel1_Cell::stringFromColumnIndex($baseCol + $fcIndex);
        }
        ...
        $lc = PHPExcel1_Cell::stringFromColumnIndex($baseCol + $lcIndex);

        // bit: 14; mask 0x4000; (1 = relative column index, 0 = absolute column index)
        if (!(0x4000 & self::getInt2d($subData, 6))) {
            // absolute column index
            $lc = PHPExcel1_Cell::stringFromColumnIndex($lcIndex);
            $lc = '$' . $lc;
        } else {
            // column offset
            $lcIndex = ($lcIndex <= 127) ? $lcIndex : $lcIndex - 256;
     ...
     * in which case an PHPExcel1_Reader_Exception is thrown
     *
     * @param int $index
     * @return string|false
     * @throws PHPExcel1_Reader_Exception
            ...
            switch ($type) {
                case 'internal':
                    ...
                    if ($this->ref[$index]['firstSheetIndex'] == 0xFFFF or $this->ref[$index]['lastSheetIndex'] == 0xFFFF) {
                        throw new PHPExcel1_Reader_Exception('Deleted sheet reference');
                    }
                ...
                default:
                    ...
                    throw new PHPExcel1_Reader_Exception('Excel5 reader only supports internal sheets in fomulas');
                    break;
            }
        }
        return false;
    ...
    private static function readBIFF8Constant($valueData)
    {
        ...
        switch ($identifier) {
            ...
            case 0x10: // error code
                // offset: 1; size: 1; error code
                $value = PHPExcel1_Reader_Excel5_ErrorCode::lookup(ord($valueData[1]));
                $size = 9;
                break;
        }
        return array(
    ...
    private static function encodeUTF16($string, $compressed = '')
    {
        ...
        return PHPExcel1_Shared_String::ConvertEncoding($string, 'UTF-8', 'UTF-16LE');
    }
    ...
    {
        return PHPExcel1_Shared_String::ConvertEncoding($string, 'UTF-8', $this->codepage);
    }
