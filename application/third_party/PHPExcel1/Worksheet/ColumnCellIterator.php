...
/**
 * PHPExcel1_Worksheet_ColumnCellIterator
 *
 ...
 */
class PHPExcel1_Worksheet_ColumnCellIterator extends PHPExcel1_Worksheet_CellIterator implements Iterator
{
    /**
     * Column index
     *
     ...
     */
    public function __construct(PHPExcel1_Worksheet $subject = null, $columnIndex = 'A', $startRow = 1, $endRow = null)
    {
        // Set subject
        $this->subject = $subject;
        $this->columnIndex = PHPExcel1_Cell::columnIndexFromString($columnIndex) - 1;
     ...
     *
     * @param integer    $startRow    The row number at which to start iterating
     * @return PHPExcel1_Worksheet_ColumnCellIterator
    ...
    public function seek($row = 1)
    {
        if (($row < $this->startRow) || ($row > $this->endRow)) {
            throw new PHPExcel1_Exception("Row $row is out of range ({$this->startRow} - {$this->endRow})");
        } elseif ($this->onlyExistingCells && !($this->subject->cellExistsByColumnAndRow($this->columnIndex, $row))) {
            throw new PHPExcel1_Exception('In "IterateOnlyExistingCells" mode and Cell does not exist');
        }
        $this->position = $row;
     ...
     *
     * @return PHPExcel1_Worksheet_Row
     */
    ...
    public function prev()
    {
        if ($this->position <= $this->startRow) {
            throw new PHPExcel1_Exception("Row is already at the beginning of range ({$this->startRow} - {$this->endRow})");
        }
    ...
    protected function adjustForExistingOnlyRange()
    {
        if ($this->onlyExistingCells) {
            ...
            if ($this->startRow > $this->endRow) {
                throw new PHPExcel1_Exception('No cells exist within the specified range');
            }
