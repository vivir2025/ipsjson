...
/**
 * PHPExcel11_CachedObjectStorage_Memcache
 *
 ...
 */
class PHPExcel11_CachedObjectStorage_Memcache extends PHPExcel11_CachedObjectStorage_CacheBase implements PHPExcel11_CachedObjectStorage_ICache
{
    /**
        ...
        if ($this->currentCellIsDirty && !empty($this->currentObjectID)) {
            ...
            if (!$this->memcache->replace($this->cachePrefix . $this->currentObjectID . '.cache', $obj, null, $this->cacheTime)) {
                if (!$this->memcache->add($this->cachePrefix . $this->currentObjectID . '.cache', $obj, null, $this->cacheTime)) {
                    $this->__destruct();
                    throw new PHPExcel11_Exception("Failed to store cell {$this->currentObjectID} in MemCache");
                }
            }
    ...
    public function isDataSet($pCoord)
    {
        ...
        if (parent::isDataSet($pCoord)) {
            ...
            if ($success === false) {
                ...
                throw new PHPExcel11_Exception('Cell entry '.$pCoord.' no longer exists in MemCache');
            }
            return true;
        }
        return false;
        ...
        foreach ($cacheList as $cellID) {
            if ($cellID != $this->currentObjectID) {
                ...
                if ($obj === false) {
                    ...
                    throw new PHPExcel11_Exception("Cell entry {$cellID} no longer exists in MemCache");
                }
                if (!$this->memcache->add($newCachePrefix . $cellID . '.cache', $obj, null, $this->cacheTime)) {
                    $this->__destruct();
                    throw new PHPExcel11_Exception("Failed to store cell {$cellID} in MemCache");
                }
            }
        }
     ...
     *
     * @param    PHPExcel11_Worksheet    $parent        The worksheet for this cell collection
     * @param    array of mixed        $arguments    Additional initialisation arguments
     */
    public function __construct(PHPExcel11_Worksheet $parent, $arguments)
    {
        ...
        if (is_null($this->cachePrefix)) {
            ...
            if (!$this->memcache->addServer($memcacheServer, $memcachePort, false, 50, 5, 5, true, array($this, 'failureCallback'))) {
                throw new PHPExcel11_Exception("Could not connect to MemCache server at {$memcacheServer}:{$memcachePort}");
            }
    ...
    {
        throw new PHPExcel11_Exception("memcache {$host}:{$port} failed");
    }
