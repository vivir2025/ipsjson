...
/**
 * PHPExcel11_CachedObjectStorage_Wincache
 *
 ...
 */
class PHPExcel11_CachedObjectStorage_Wincache extends PHPExcel11_CachedObjectStorage_CacheBase implements PHPExcel11_CachedObjectStorage_ICache
{
    /**
        ...
        if ($this->currentCellIsDirty && !empty($this->currentObjectID)) {
            ...
            if (wincache_ucache_exists($this->cachePrefix.$this->currentObjectID.'.cache')) {
                if (!wincache_ucache_set($this->cachePrefix.$this->currentObjectID.'.cache', $obj, $this->cacheTime)) {
                    $this->__destruct();
                    throw new PHPExcel11_Exception('Failed to store cell '.$this->currentObjectID.' in WinCache');
                }
            } else {
    ...
    public function isDataSet($pCoord)
    {
        ...
        if (parent::isDataSet($pCoord)) {
            ...
            if ($success === false) {
                ...
                throw new PHPExcel11_Exception('Cell entry '.$pCoord.' no longer exists in WinCache');
            }
            return true;
        }
        return false;
        ...
        foreach ($cacheList as $cellID) {
            if ($cellID != $this->currentObjectID) {
                ...
                if ($success === false) {
                    ...
                    throw new PHPExcel11_Exception('Cell entry '.$cellID.' no longer exists in Wincache');
                }
                if (!wincache_ucache_add($newCachePrefix.$cellID.'.cache', $obj, $this->cacheTime)) {
                    $this->__destruct();
                    throw new PHPExcel11_Exception('Failed to store cell '.$cellID.' in Wincache');
                }
            }
        }
