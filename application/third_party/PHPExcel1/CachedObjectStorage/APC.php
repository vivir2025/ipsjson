...
/**
 * PHPExcel11_CachedObjectStorage_APC
 *
 ...
 */
class PHPExcel11_CachedObjectStorage_APC extends PHPExcel11_CachedObjectStorage_CacheBase implements PHPExcel11_CachedObjectStorage_ICache
{
    /**
    ...
    protected function storeData()
    {
        if ($this->currentCellIsDirty && !empty($this->currentObjectID)) {
            ...
            if (!apc_store(
                ...
                $this->__destruct();
                throw new PHPExcel11_Exception('Failed to store cell ' . $this->currentObjectID . ' in APC');
            }
    ...
    public function isDataSet($pCoord)
    {
        ...
        if (parent::isDataSet($pCoord)) {
            ...
            if ($success === false) {
                ...
                throw new PHPExcel11_Exception('Cell entry '.$pCoord.' no longer exists in APC cache');
            }
            return true;
        }
        return false;
        ...
        foreach ($cacheList as $cellID) {
            if ($cellID != $this->currentObjectID) {
                ...
                if ($obj === false) {
                    ...
                    throw new PHPExcel11_Exception('Cell entry ' . $cellID . ' no longer exists in APC');
                }
                if (!apc_store($newCachePrefix . $cellID . '.cache', $obj, $this->cacheTime)) {
                    $this->__destruct();
                    throw new PHPExcel11_Exception('Failed to store cell ' . $cellID . ' in APC');
                }
            }
        }
